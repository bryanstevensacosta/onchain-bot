...
export interface SeedChannelEntry {
  channelId: string;
  username?: string;
  title?: string;
}
...
/**
 * Parse INGESTION_TELEGRAM_SEED_CHANNELS env var (format: "id|username|title,...").
 */
function parseSeedChannels(raw: string | undefined): SeedChannelEntry[] {
  if (!raw) return [];
  return raw
    .split(',')
    ...
    .map((entry) => {
      ...
      if (!channelId) {
        throw new Error(
          `Invalid INGESTION_TELEGRAM_SEED_CHANNELS entry: "${entry}". Expected at least a channelId.`,
        );
      }
      const username = parts[1] || undefined;
      const title = parts[2] || undefined;
      const out: SeedChannelEntry = { channelId };
...
export const appConfig = registerAs(
  ...
  (): AppConfig => ({
    ...
    database: {
      ...
      username: process.env.POSTGRES_USER ?? 'onchain',
      password: process.env.POSTGRES_PASSWORD ?? 'onchain',
