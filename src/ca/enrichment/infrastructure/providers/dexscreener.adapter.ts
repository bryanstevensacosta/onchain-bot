 ...
 * Supports all chains (EVM + Solana + most L2s + Sui).
 *
 * Returns the first matching token's data (DexScreener returns pairs
 * across all DEXes for the same token).
 */
@Injectable()
export class DexScreenerAdapter extends MarketDataProviderPort {
  ...
  public async fetch(
    ...
    if (!this.supportedChains.includes(chain)) return null;
    try {
      ...
      return this.toMarketData(data.pairs);
    } catch (err) {
  ...
  private toMarketData(pairs: DexScreenerPair[]): MarketData {
