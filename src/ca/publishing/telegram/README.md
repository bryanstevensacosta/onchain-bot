...
> Cierra el pipeline CA: toma calls APPROVED por `filters`, los formatea como mensaje de Telegram y los envía a uno o más canales de output (mock por defecto, MTProto real opcional).

Forma parte de `src/ca/publishing/telegram/` y se monta vía `TelegramPublishingModule` (`publishing.module.ts:57`).

---

## 1. Propósito

Este BC es el **terminal del pipeline CA**. Una vez que un call pasa los filtros (`filters.token.approved`), este BC:
