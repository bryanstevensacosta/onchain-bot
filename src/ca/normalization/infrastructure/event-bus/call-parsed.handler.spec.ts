...
import { CallParsedEvent } from 'ca/parsing/domain/events/call-parsed.event';
...
describe('CallParsedHandler', () => {
  ...
  it('calls normalize.execute with reconstructed input', async () => {
    ...
    const handler = new CallParsedHandler({ execute } as never);
    ...
    expect(arg.channelId).toBe('chan-A');
