...
import {
  ...
  EvaluateCallInput,
} from 'ca/analytics/domain/ports/performance-evaluator.port';
...
/**
 * v1 Performance evaluator — uses DexScreener's current price + FDV
 ...
 */
@Injectable()
export class DexScreenerPerformanceEvaluatorAdapter extends PerformanceEvaluatorPort {
  private readonly logger = new Logger(
    DexScreenerPerformanceEvaluatorAdapter.name,
  );
  ...
  public async evaluateCall(
    ...
    try {
      const { data } = await axios.get<DexScreenerResponse>(
        `${DexScreenerPerformanceEvaluatorAdapter.ENDPOINT}/${input.address}`,
        { timeout: 5000 },
      );
