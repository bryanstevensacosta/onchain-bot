...
/**
 * A Telegram output channel where approved calls are republished.
 *
 * `tier`:
 * - PRIMARY:   all approved calls go here (default tier)
 * - SECONDARY: only high-confidence calls (score >= 70)
 * - PREMIUM:   only very high-confidence calls (score >= 80), rate-limited
 *
 * v1 uses a static list; v2 will use database-backed config.
 */
