import {
  CanonicalTokenCall,
  MentionInput,
} from 'discovery/normalization/domain/entities/canonical-token-call.entity';
  ...
  describe('mergeWith', () => {
    ...
    it('adds a new source when different channel mentions the token', () => {
      const call = CanonicalTokenCall.create(
        buildMention({ channelId: 'chan-A' }),
      );
      const merged = call.mergeWith(
        buildMention({ channelId: 'chan-B', messageId: 5 }),
      );
      expect(merged.mentionCount).toBe(2);
      expect(merged.sourceCount).toBe(2);
      expect(merged.sources.map((s) => s.channelId).sort()).toEqual([
        'chan-A',
        'chan-B',
      ]);
    });
    ...
    it('keeps firstSeenAt as min, lastSeenAt as max', () => {
      ...
      const merged = call.mergeWith(
        buildMention({ occurredAt: t0, messageId: 2, channelId: 'chan-B' }),
      );
      const merged2 = merged.mergeWith(
        buildMention({ occurredAt: t2, messageId: 3, channelId: 'chan-C' }),
      );
    ...
    it('replaces ticker with higher-confidence mention', () => {
      ...
      const merged = call.mergeWith(
        buildMention({ ticker: 'RIGHT', confidence: 0.9, channelId: 'chan-B' }),
      );
    ...
    it('keeps existing ticker when new mention has lower confidence', () => {
      ...
      const merged = call.mergeWith(
        buildMention({ ticker: 'BAD', confidence: 0.2, channelId: 'chan-B' }),
      );
    ...
    it('keeps existing ticker when new mention has no ticker', () => {
      ...
      const merged = call.mergeWith(
        buildMention({ ticker: null, channelId: 'chan-B' }),
      );
    ...
    it('takes new ticker when existing is null', () => {
      ...
      const merged = call.mergeWith(
        buildMention({ ticker: 'NEW', channelId: 'chan-B' }),
      );
      expect(merged.ticker).toBe('NEW');
    });
    ...
    it('takes new name when existing is null', () => {
      ...
      const merged = call.mergeWith(
        buildMention({ name: 'pepecoin', channelId: 'chan-B' }),
      );
