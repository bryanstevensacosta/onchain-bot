...
import { Source } from 'discovery/normalization/domain/value-objects/source.vo';
 ...
 * - Sources: deduplicate by channelId, accumulating messageIds.
...
export class CanonicalTokenCall extends AggregateRoot<string> {
  ...
  public static create(input: MentionInput): CanonicalTokenCall {
    const identity = TokenIdentity.create(input.chain, input.address);
  ...
  public get identity(): TokenIdentity {
...
function mergeSources(
  ...
  const idx = existing.findIndex((s) => s.channelId === mention.channelId);
  if (idx === -1) {
    return [
      ...existing,
      Source.firstMention(
