...
interface ChannelReputationStatsProps {
  ...
  readonly avgAthMultiple: number | null; // average peak multiplier
  readonly confidence: ConfidenceLevel;
  readonly lastEvaluatedAt: Date;
}

/**
 * Aggregated reputation statistics for a Telegram channel.
 *
 ...
 */
export class ChannelReputationStats extends ValueObject<ChannelReputationStatsProps> {
  protected constructor(props: ChannelReputationStatsProps) {
    super(props);
  }
  ...
  public static fromValues(input: {
    ...
    return new ChannelReputationStats({
      ...input,
      lastEvaluatedAt: input.lastEvaluatedAt ?? new Date(),
    });
  }

  public static empty(channelId: string): ChannelReputationStats {
  ...
  public get confidence(): ConfidenceLevel {
