...
| `INGESTION_PORT`     | **Nothing**: documented in `.env.example` but no reader in `src`. Keep all three aligned to `3031` (prod host `3032` → container `3031`, twin host `3033` → container `3031` on the droplet) until the vars are unified |
