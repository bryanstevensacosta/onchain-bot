...
- **PRIMARY PATH (SSE, real-time):** `ProcessFeedMessageHandler`
  (`apps/backend/src/telegram/crypto-news-integration/application/handlers/process-feed-message.handler.ts`)
...
- `FilteredFeedService.getMatchingMessages()` — fetch RAW content
...
ingestion-telegram (persists RAW, stores media, emits SSE metadata)
    ...
    |    |                                                             |
    |    v                                                             |
    |  ProcessFeedMessageHandler.handle(raw)                     |
...
- Reads `app.ingestion.useSseFeed` (`USE_SSE_CRYPTO_NEWS`,
...
- Cron expression: `` `*/${intervalMinutes} * * * *` `` under the
  `SchedulerRegistry` job name `feed-polling`.
...
- **SLO:** `Date.now() - ingestedAt < 10_000ms`, measured in
  `ProcessFeedMessageHandler.logLatency()` after a successful enqueue.
...
- `ProcessFeedMessageHandler.handle()` wraps everything in
  try/catch, logs `Failed to process feed message
...
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Dynamic interval 1-vs-5 min, `FETCH_LIMIT=50`, job name `feed-polling` | `enqueue-matching-cron.scheduler.ts` `onApplicationBootstrap()`                                                                 |
| Dedup-before-fetch, window 10, 10_000ms threshold, never throws        | `process-feed-message.handler.ts` `handle()` + `logLatency()`                                                                   |
