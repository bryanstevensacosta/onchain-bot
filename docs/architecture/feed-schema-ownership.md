...
code that references a feed channel therefore stores the channel id
...
- `docs/architecture/feed-dual-path.md`
...
against feed sources` and warns when a channel has no filters.
...
- `1860000000001-DropIngestionOwnedFeedTables.ts` — the split: drops
...
- Never re-add a `@ManyToOne` / FK from `ChannelContentFilterConfigEntity`
  or `PublisherQueueEntity` to a feed table. If a future reader is
