...
- Telegram channel ID (e.g. `-1001234567890`). The channel must be **public**
  (username-based, e.g. `@cryptonewschannel`) or **private with the ingestion
...
> backend `CryptoNewsController` exposes only filter-rule CRUD). Do not call it:
> it writes nothing. Always create sources on ingestion-telegram as above.

## Step 2. Configure keywords (backend)

Keywords live in the backend (`crypto-news-publisher` BC). A message is
...
`CryptoNewsController` still owns. Note the path shape: create/list are nested
under the source, update/delete are not.

```bash
# Create (201)
curl -X POST http://localhost:3030/crypto-news/sources/-1001234567890/filters \
  -H 'Content-Type: application/json' \
  -d '{
    "pattern": "\\s+",
    "replacement": " ",
...
curl http://localhost:3030/crypto-news/sources/-1001234567890/filters

# Update (PUT, not PATCH)
curl -X PUT http://localhost:3030/crypto-news/filters/<id> \
  -H 'Content-Type: application/json' -d '{"isActive": false}'

# Toggle active flag
curl -X PATCH http://localhost:3030/crypto-news/filters/<id>/toggle

# Delete (204)
curl -X DELETE http://localhost:3030/crypto-news/filters/<id>
```
...
```bash
curl "http://localhost:3031/api/crypto-news/messages/channel/-1001234567890?limit=5"
```

If empty after 30+ s, check the source row (`GET /api/crypto-news/sources`),
...
(`FilteredCryptoNewsService` also logs
...
curl -X PATCH http://localhost:3031/api/crypto-news/sources/-1001234567890/toggle
...
(`CryptoNewsRetentionCleanupScheduler`), so deleting a source does not
...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | `POST {INGESTION}/api/crypto-news/sources`                                                                                                                                            | `apps/ingestion-telegram/src/telegram/crypto-news/api/http/crypto-news.controller.ts` (`@Post('sources')`)                            |
| 1    | `GET {INGESTION}/api/crypto-news/sources`                                                                                                                                             | same file (`@Get('sources')`)                                                                                                         |
| 1    | `GET {INGESTION}/api/crypto-news/sources/active/ids`                                                                                                                                  | same file                                                                                                                             |
| 1    | `GET {INGESTION}/api/crypto-news/stats`                                                                                                                                               | same file (`@Get('stats')`)                                                                                                           |
| 1    | `PATCH {INGESTION}/api/crypto-news/sources/:channelId` (title/handle only)                                                                                                            | same file (`@Patch('sources/:channelId')`)                                                                                            |
| 1/6  | `PATCH {INGESTION}/api/crypto-news/sources/:channelId/toggle`                                                                                                                         | same file                                                                                                                             |
| 6    | `DELETE {INGESTION}/api/crypto-news/sources/:channelId`                                                                                                                               | same file (`@Delete('sources/:channelId')`)                                                                                           |
| 5    | `GET {INGESTION}/api/crypto-news/messages?limit=50` (wrapped `{timestamp,count,data}`, max 200)                                                                                       | same file (`@Get('messages')`)                                                                                                        |
| 5    | `GET {INGESTION}/api/crypto-news/messages/channel/:channelId?limit=50`                                                                                                                | same file                                                                                                                             |
| 2    | `GET/POST /crypto-news-publisher/keywords`, `GET/PATCH/DELETE /crypto-news-publisher/keywords/:id`, `POST /crypto-news-publisher/keywords/batch`                                      | `apps/backend/src/telegram/crypto-news-publisher/api/http/keywords.controller.ts` (`@Controller('crypto-news-publisher/keywords')`)   |
| 3    | `GET/POST /crypto-news-publisher/blacklist`, `GET/PATCH/DELETE /crypto-news-publisher/blacklist/:id`, `POST /crypto-news-publisher/blacklist/batch`                                   | `apps/backend/src/telegram/crypto-news-publisher/api/http/blacklist.controller.ts` (`@Controller('crypto-news-publisher/blacklist')`) |
...
| 5    | `GET/PATCH /crypto-news-publisher/llm/config` (`llmEnabled`, `publishingEnabled`)                                                                                                     | `apps/backend/src/telegram/crypto-news-publisher/api/http/llm-config.controller.ts` (`@Controller('crypto-news-publisher/llm')`)      |
