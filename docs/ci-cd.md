...
- **Self-hosted runner isolation**: `Deploy to *` jobs run on the droplet's
  ...
  *built artifacts* + `node_modules` for migrations, never the source build.
- **release-please triggers on `push` to master**, *after* CI has gone green
...
artifacts are *not* consumed by the CD workflows — the self-hosted runner
...
`environment: production` (enforcement is *not* via GitHub environment
...
Triggers on `push` to `master` *in parallel* with the prod deploy. The
...
merge commit then triggers another `release-please` run which *creates* the
...
The squash-merge creates a *new* commit on master (no history rewriting), and
...
4. **Manual media prune (medium risk).** Force the retention scheduler to
   run *now* and shrink the uploads tree. Use this if disk is critical
   ...
   This is *not* the same as the scheduler — it ignores
...
removes all dangling images, but it also removes *untagged* images that
