...
`DropFeedSourcesResidual1790200000000`
(`apps/ingestion-telegram/.../migrations/1790200000000-DropFeedSourcesResidual.ts`):
...
public async up(queryRunner: QueryRunner): Promise<void> {
  ...
  if (!Number.isFinite(count)) {
    throw new Error(
      'DropFeedSourcesResidual: could not count crypto_news_sources rows — aborting',
    );
  }
  if (count !== 0) {
    throw new Error(
      `DropFeedSourcesResidual: crypto_news_sources still holds ${count} row(s) — aborting (migrate rows to telegram_feed_sources first)`,
    );
  }
