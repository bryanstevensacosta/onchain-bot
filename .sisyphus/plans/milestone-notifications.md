...
# ADDENDUM: Published Call Tracking for Gate/Filters

> **Date**: 2024-06-24 (added after initial plan)
> **User Request**: "Necesitamos tracking de las calls de los KOLs que publicamos en VIP channels para configurar en DB + frontend + Redis, del tracking, si por ejemplo quisieramos configurar que solo tracamos las monedas que hayan tenido un milestone en las ultimas 72h, o que hayan caido +90% del mc del snapshot"

## Scope del Addendum

El tracking de calls publicadas permite:
1. **Gate futuro**:Solo permitir re-publicar calls que cumplan criterios (ej: "solo calls que hitearon 2x en últimas 72h")
2. **Analytics**: Ver performance de calls publicadas (cuantas hitearon milestones, cuantas cayeron -90%)
3. **Filtros configurables via DB**: `SettingsFilterEntity` con nuevo tipo `published_call_tracking`

### Entidad nueva: TrackedPublishedCall

Ubicación: `apps/backend/src/token/call-tracking/domain/entities/tracked-published-call.entity.ts`

```ts
interface TrackedPublishedCallProps {
  readonly kolId: string;                    // KOL que publicó
  readonly chain: string;
  readonly address: string;
  readonly ticker: string | null;
  readonly mcAtPublish: number;              // MC cuando se publicó
  readonly mcNow: number | null;              // MC actual (actualizado por cron)
  readonly milestonesHit: ReadonlyArray<number>; // ej [2, 3, 5]
  readonly maxMilestone: number | null;       // el milestone más alto hit
  readonly priceDropPercent: number | null;   // ej -95.5 para 95.5% drop
  readonly publishedAt: Date;
  readonly lastUpdatedAt: Date;
  readonly isActive: boolean;                 // para cleanup
}
```

### Flujo de tracking

1. **On publish** (vip-calls-channel): Cuando se publica una call, se emite evento `publishing.telegram.published`
2. **Handler** (call-tracking): Suscribe a `publishing.telegram.published` → crea `TrackedPublishedCall` con `mcAtPublish`, `milestonesHit: []`, `priceDropPercent: null`
3. **Live cron** (NEW): Cada 5 min, actualiza `mcNow`, recalcula `milestonesHit` (del milestone BC), `priceDropPercent`, `maxMilestone`
4. **Gate use case**: Para re-publicar, consulta `TrackedPublishedCall` contra settings `published_call_tracking` filters

### Settings filters

En `SettingsFilterEntity`, agregar nuevo tipo `published_call_tracking`:

| Setting | Value | numericValue | Descripción |
|---|---|---|---|
| `milestone_min_hours_ago` | null | 72 | Solo considerar calls que hitearon milestone hace ≤ N horas |
| `milestone_min_multiple` | null | 2 | Solo calls que hitearon ≥ Nx |
| `price_drop_max_percent` | null | 90 | Excluir calls con caida ≥ N% (ej: 90 = excluir si cayó 90%+) |
| `tracking_enabled` | true/false | null | Toggle global |

### Frontend

**Reutilizar Live page** (`apps/frontend/src/pages/Live.tsx`):
- Agregar filtro: "Solo calls con milestones" → consulta endpoint `/call-tracking/tracked?filter=has_milestones`
- Agregar columna en tabla: "Milestone", "Price Drop %"
- Nuevo endpoint: `GET /call-tracking/tracked` con query params para filtros

### Tasks adicionales (ADDENDUM)

- [ ] T24. TrackedPublishedCall entity + TypeORM + in-memory repos
- [ ] T25. Settings filter type `published_call_tracking` + default seed
- [ ] T26. Publish handler (suscribe a `publishing.telegram.published`, crea tracked call)
- [ ] T27. Tracking cron (actualiza mcNow, milestones, priceDrop cada 5 min)
- [ ] T28. Gate use case (consulta filters + decide si call puede re-publicarse)
- [ ] T29. HTTP controller endpoints (`GET /call-tracking/tracked`, `GET /call-tracking/tracked/:chain/:address`)
- [ ] T30. Frontend: Live page extendida con filtros y columnas de tracking
- [ ] T31. Redis cache para tracking queries (optional, si performance lo requiere)

### Dependencias externas (del plan original)

- T20 (PublishedCall extend) — ya tiene `mcAtCall`, reusable
- Milestone BC (T1-T23) — provee `milestonesHit` y `mcNow` para el tracking

### Acceptance Criteria

- [ ]Nueva entidad `TrackedPublishedCall` con tabla en DB
- [ ]Settings filters configurables via API (`PUT /settings/filters` con tipo `published_call_tracking`)
- [ ]Cada publish crea un TrackedPublishedCall
- [ ]Tracking cron actualiza mcNow, milestones, priceDrop cada 5 min
- [ ]Endpoint `/call-tracking/tracked` acepta filtros: `min_milestone`, `max_price_drop`, `has_milestones`
- [ ]Frontend Live page muestra columna de "Milestone" y "Price Drop %" con filtros activos

### QA Scenarios

```
Scenario: Tracking cron actualiza mcNow y milestones
  Tool: Bash (psql + jest)
  Steps:
    1. Publicar call via /vip-calls/publish
    2. Verificar TrackedPublishedCall creada con mcAtPublish y milestonesHit=[]
    3. Ejecutar tracking cron manualmente (endpoint admin)
    4. psql: SELECT mc_now, milestones_hit FROM tracked_published_calls WHERE address='...'
    5. Assert mc_now > 0 y milestones_hit tiene valores
  Expected Result: tracking actualizado
  Evidence: .sisyphus/evidence/addendum-t27-tracking-cron.txt

Scenario: Gate filter rejects call that dropped -95%
  Tool: Bash (curl)
  Preconditions: settings filter price_drop_max_percent=90
  Steps:
    1. Insert tracked call con price_drop_percent=-95
    2. POST /call-tracking/gate-allow con {chain, address}
    3. Assert response: {allowed: false, reason: "price_drop_exceeds_limit"}
  Expected Result: gate bloquea call
  Evidence: .sisyphus/evidence/addendum-t28-gate-filter.txt
```