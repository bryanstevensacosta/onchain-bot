...
async function main(): Promise<void> {
  ...
  const existing = process.env.TELEGRAM_MTPROTO_SESSION ?? '';
  ...
  try {
    ...
    console.log('  1. Paste the line above into .env as TELEGRAM_MTPROTO_SESSION');
    console.log('  2. Restart the app: npm run start:dev');
    console.log(
      '  3. Look for "Telegram MTProto session already authorized." in logs\n',
    );
  } finally {
    rl.close();
