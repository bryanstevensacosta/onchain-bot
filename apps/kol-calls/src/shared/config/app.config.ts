...
export class ConfigValidationError extends Error {
  constructor(public readonly issues: ConfigValidationIssue[]) {
    super(
      `Invalid kol-calls config: ${issues
...
const OPTIONAL_WITH_WARNING: Array<{ envVar: string; description: string }> = [
  ...
  {
    envVar: 'KOL_CALLS_API_KEY',
    description: 'API key for feed consumers (fail-open when empty)',
  },
];

/**
 * Tier-1 validation for kol-calls config.
 ...
 */
export function validateKolCallsConfig(env: NodeJS.ProcessEnv = process.env): {
  warnings: string[];
} {
