...
NestJS 11 service (Tramo 1 of the mega-refactor) owning the KOL
hot path after the P51 split (2026-09-26): KOL mentions in → extraction →
enrichment → snapshot → tracking/rankings, plus the P51 HTTP contract
(`GET /api/mentions`, `GET /api/snapshots`, paginated + keyed +
`x-api-key`) consumed by `apps/kol-calls-publisher/`. Scoring, templates,
approval and telegram publishing MOVED to the publisher via `git mv`
(byte-identical, no behavior change) — their sections below (§SCORING,
§TEMPLATES, §APPROVAL, §PUBLISHING, §PUBLISHING VIA GATEWAY, STRUCTURE
subtrees, MODULE MAP rows) are HISTORICAL and maintained there now.
Built and staying here:
Config + `GET /api/health` (composite: ingestion/database/mentions/
snapshots/tracking since P51) + `IngestionModule` (SSE-only KOL client, P20) +
...
`TrackingModule` (first-seen
`TrackedMention` + kol +5x rating + `kol_window_stats` cron +
`GET /api/kol-rankings`, todo 12, Ph12 + P8 + P11 + P17) wired into
`AppModule`. The P51 contract readers live here: `MentionsController`
(`GET /api/mentions`, paginated + keyed, in `src/normalization/`) +
`SnapshotsController` (`GET /api/snapshots`, in `src/snapshot/`).

Design pivots (2026-09-24) that govern every future todo:

- **P1 — NO dedup of any kind in kol-calls**: repeats are first-class data
  (each mention = one row). Ingestion-telegram dedups source-side; kol-calls
...
- **P4 — identity/sources live in ingestion-telegram**: sources have 2 types
  (`kol` + `feed`). kol-calls stores NO profiles; it consumes sources
...
- **P10 — strict type separation**: kol-calls subscribes ONLY to
...
- **P27 — snapshots in own module, SAME DB** (planned — see SNAPSHOTS):
  `src/snapshot/` with its own tables inside the kol-calls DB (no separate
...
- **P12-bis/P13 — Dexter lookup is NOT here**: bot lookup lives in
  `apps/dexter-onchain-bot/` (Tramo 3), fed by `apps/market-data`; kol-calls
...
- **C-SSE-01 — strict type filtering**: SSE frames carry
  `data.messageType: 'kol'|'crypto-news'`; kol-calls subscribes ONLY to
...
flowchart TB
    ...
    subgraph APP["kol-calls - the factory (this app)"]
...
- **Sorter (ear)** — IN: fresh messages, OUT: messages labelled tip or news. Tips go to kol-calls, news goes to feed-publisher; each side ignores the other type (P3, P10).
...
> One node per kol-calls BC in process order. `(BUILT)` = wired in
...
Order: kol-calls → feed-publisher → market-data (+ `dexter-onchain-bot`
...
| 1 · kol-calls    | `.omo/plans/mega-refactor-kol-system.md`     | this app (16 todos)             |
...
as PLANNED (not yet scaffolded; only `apps/kol-calls/` exists).

## COMMANDS

```bash
# In apps/kol-calls/
npm run dev                # nest start --watch (alias of start:dev, port KOL_CALLS_PORT, default 3050)
npm run start:dev          # nest start --watch (port KOL_CALLS_PORT, default 3050)
...
`KOL_CALLS_PORT=3059` and curling `GET /api/health` → 200 +
...
```text
src/
├── main.ts                       # bootstrap() — ValidationPipe whitelist/forbidNonWhitelisted/transform, listen KOL_CALLS_PORT ?? 3050
...
│   ├── application/ports/mention-snapshot.repository.ts  # save/findByMentionId/count (same kol-calls DB, no separate DB)
...
│   ├── guards/api-key.guard.ts (+ spec)       # P50: 401 on missing/wrong key, fail-open when KOL_CALLS_API_KEY empty; on ALL controllers except health
...
├── health.e2e-spec.ts
└── jest-e2e.json
Root: package.json (@onchain-bot/kol-calls v0.1.0), nest-cli.json (deleteOutDir),
...
Wired today (post-P51 split 2026-09-26): `ConfigModule` (global, `.env.dev` > `.env`) + `HealthModule` +
...
return — wired 2026-09-25; P51: plus `MentionsController`
`GET /api/mentions` paginated + keyed) + `EnrichmentModule` (todo 8, MarketDataPort
...
same kol-calls DB — wired 2026-09-25; P51: plus `SnapshotsController`
`GET /api/snapshots` paginated + keyed) + `TrackingModule`
(todo 12, first-seen + rating + rankings per Ph12 + P8 + P11 + P17,
direct call fix-1, `TrackedMention` + tracking cron + `GET
/api/kol-rankings` — the rating endpoint stays here, P51).

Moved to `apps/kol-calls-publisher/` in the P51 split (2026-09-26, `git mv`,
byte-identical — maintained there now, rows below are historical):
`ScoringModule` (todo 9 + todo 22 P28) + `TemplatesModule` (todo 10:
orchestrator cron 1 min, 4-strategy ranking, 12-endpoint controller,
threads 501 stub, `telegram_bots` catalog, `vip-calls` seed) +
`ApprovalModule` (todo 11, per-template bouncer) + `TelegramModule`
(todo 11 Ph11 + C2 + gateway todo 4, catalog token per call).
...
event. Justification: kol-calls wires no event bus at this stage; the head
...
`aggregateId` = mention id) is returned directly — kol-calls wires no
...
→ INSUFFICIENT_DATA → CHAIN_UNSUPPORTED (`evm|solana`, kol-calls
...
`grep -r classified_calls apps/kol-calls/src` is empty). The full
...
(empty `KOL_CALLS_API_KEY`); with a key set, missing/wrong keys throw 401
...
OWNED here (P27): same kol-calls DB as the mention index (joins stay
...
| ------------------------------- | ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `KOL_CALLS_ENABLED`             | `false`                                                      | master switch                                                                                                                                                                                                                                  |
...
Tier-1 validation (`validateKolCallsConfig`): `ENCRYPTION_KEY` +
...
Owner of migrations is kol-calls itself (own
`data-source.ts` + `migration:run`); snapshot tables (`mention_snapshots`) live
in THIS db (P27), never a separate base.

Port discrepancy to know: `main.ts` listens on `KOL_CALLS_PORT ?? 3050`,
while `buildAppConfig().port` reads `PORT ?? 3030`. Canonical runtime port is
`KOL_CALLS_PORT=3050` (main.ts wins); do not set bare `PORT` expecting 3050.

## HEALTH

`GET /api/health` → 200 + `{ status: 'ok', components }` (composite
shape since todo 15, C3 staging harness; todo 2 was the static stub).
P51 split (2026-09-26): `components` carries `ingestion`, `database`,
`mentions`, `snapshots` and `tracking` (tracking via its P21 hook-point
indicator; mentions/snapshots are the P51 contract reads, static `up` +
detail). Templates + publishing health moved with their modules to
`apps/kol-calls-publisher/` (its composite carries `kol-calls` +
`database` + `scoring` + `templates` + `approval` + `publishing`).
Pre-split, components were `ingestion`/`database`/`templates`/
`publishing` (todo 15 acceptance jqs each key — that acceptance now
lives in the publisher). Ingestion (SSE-only client, P20 — no indicator
class exists yet) and database (in-memory repos until the persistence
todo) report static `up` with an explicit `detail` string. Deep probes
...
component `tracking`)
is consumed by the P51 composite (`new TrackingHealthIndicator().check()`
in `HealthController`); the rest stay provided + exported, unwired until
the persistence todo (deep probes, gap narrowed 2026-09-25). The moved
indicators (scoring/templates/approval/telegram) are consumed by the
publisher composite now.
...
harness `apps/kol-calls/scripts/dual-run-compare.mjs` (5% threshold,
...
`MentionSnapshot` + `MentionSnapshotRepository`, SAME kol-calls DB,
...
Explicitly NOT in kol-calls: `feed` (feed-publisher, P10),
...
4. `buildAppConfig().port` reads `PORT ?? 3030` while `main.ts` uses
   `KOL_CALLS_PORT ?? 3050` — bare `PORT` will mislead.
...
10. `bs58` is a declared kol-calls dep (Solana validation, backend-mirror
...
11. RESOLVED 2026-09-25 (todo 10) — `src/templates/` + `PublishingTemplate`
    ...
    `eslint.config.*` in kol-calls yet — `npm run lint` errors repo-wide
    (pre-existing; gates here are prettier + tsc + jest).

## DECISIONS (P1–P29 + contracts — one line each, 2026-09-24)

- P1 (2026-09-24): no dedup of any kind in kol-calls; repeats are first-class rows.
...
- P12-bis (2026-09-24): per-template publishing bots stay in kol-calls (unchanged by P13).
...
- P27 (2026-09-24): snapshots in own module `src/snapshot/`, SAME kol-calls DB (default, a veto).
...
the new path (`apps/kol-calls/...`) plus the refactor target; nothing
