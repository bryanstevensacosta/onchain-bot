...
/**
 * HMAC-SHA256 request signer for the telegram-bots-gateway (P42).
 *
 * Canonical string (gateway `auth/application/hmac.service.ts`):
 ...
 * and `rawBody` the exact JSON bytes sent.
 *
 * Keyless dev (no `BOTS_GATEWAY_CLIENT_ID` or no
 * `BOTS_GATEWAY_CLIENT_SECRET`): `authHeaders()` returns `{}` — the
 * gateway guard fails open when no clients are registered. Secrets and
 * signatures are never logged here.
 */
@Injectable()
export class GatewayHmacSigner {
  ...
  private clientId(): string {
    try {
      return (
        this.config?.get<string>('BOTS_GATEWAY_CLIENT_ID', '') ?? ''
      ).trim();
    } catch {
      return '';
    }
