...
/**
 * Gateway-backed sender (P42: the ONLY transport in this app).
 *
 * Text chunking: 4096-char message chunks, photo + 1024-char caption
 * + overflow chunks. Each chunk is one gateway
 * `POST /api/bots/:id/send` (`message` or `photo` or `media_group`,
 * per-chunk `client_msg_id` for idempotency). Local-file shapes
 * (disk paths), video and `reply_markup` have NO gateway equivalent —
 * they are rejected fail-closed so the caller holds the post (never a
 * silent drop, never a direct Bot API leg). The plaintext token never
 * appears here — only the vault `botId` travels. Uses global `fetch`.
 */
@Injectable()
export class GatewaySendClient extends SchedulingGatewaySenderPort {
  ...
  private baseUrl(): string {
    try {
      const raw =
        this.config?.get<string>('BOTS_GATEWAY_URL', '') ??
        'http://localhost:4070';
