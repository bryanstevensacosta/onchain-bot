...
vi.mock('@/entities/feed/model/use-feed', () => ({
  useFeedMessages: vi.fn(),
...
import {
  ...
  useUpdateKeyword,
  useDeleteKeyword,
} from '@/features/feed-publisher/model/use-keywords';
import {
  useQueue,
  useQueueCounts,
} from '@/features/feed-publisher/model/use-queue';
import { FeedPage, TRUNCATION_LIMIT } from '../index';
import type { FeedMessage, FeedSource } from '@/entities/feed/api/feed-queries';
import type { KeywordView } from '@/features/feed-publisher/api/keywords-api';
import type {
  QueueCountsView,
  QueueEntryView,
...
function makeMessagesQuery(data: ReadonlyArray<FeedMessage>) {
  return {
    data,
    isLoading: false,
    error: null,
  } as unknown as ReturnType<typeof useFeedMessages>;
}

function makeSourcesQuery(data: ReadonlyArray<FeedSource>) {
  return {
    data,
    isLoading: false,
    error: null,
...
describe('FeedPage — media rendering', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('renders <img> for each media item with correct src and alt', () => {
    const msgWithMedia: FeedMessage = {
      id: 'msg-1',
      channelId: 'WatcherGuru',
      messageId: 42,
  ...
  it('renders <video> for media with type video even when mimeType is application/octet-stream', () => {
    const msgWithVideo: FeedMessage = {
      id: 'msg-video',
      channelId: 'WatcherGuru',
      messageId: 45,
      title: 'Video message',
    ...
    const { container } = renderWithClient(<FeedPage />);
  ...
  it('renders no <img> when media array is empty', () => {
    const msgWithoutMedia: FeedMessage = {
      id: 'msg-2',
      channelId: 'WatcherGuru',
      messageId: 43,
  ...
  it('handles backward-compat case where media is undefined (no crash, no <img>)', () => {
    ...
    // Must not throw
    expect(() => renderWithClient(<FeedPage />)).not.toThrow();
...
describe('FeedPage — source handle/link rendering', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders message with source handle as clickable link to public Telegram URL', () => {
    const sourceWithHandle: FeedSource = {
      channelId: '123',
      handle: '@test-handle',
      title: 'Test Source',
      isActive: true,
    ...
    const msgWithHandle: FeedMessage = {
      id: 'msg-1',
      channelId: '123',
      messageId: 5,
      title: 'Test Message',
  ...
  it('renders message with source no handle (private channel) using c/ URL format', () => {
    const sourceNoHandle: FeedSource = {
      channelId: '123',
      handle: null,
      title: 'Test Channel',
      isActive: true,
...
describe('FeedPage — formatting entities rendering', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockedUseSources.mockReturnValue(makeSourcesQuery([baseSource]));
  });
  ...
  it('renders text without entities as plain text (no anchor tags)', () => {
    ...
    const msgPlainText: FeedMessage = {
      id: 'msg-plain',
      channelId: '123456',
      messageId: 101,
...
describe('FeedPage — expand/collapse', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockedUseSources.mockReturnValue(makeSourcesQuery([baseSource]));
  });
  ...
  it('c) boundary TRUNCATION_LIMIT (500 chars) has NO button; TRUNCATION_LIMIT+1 (501) HAS a button', () => {
    ...
    const { rerender } = renderWithClient(<FeedPage />);
...
describe('FeedPage — 24h window (Todo 2: feed-24h-window-media-retention)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockedUseSources.mockReturnValue(makeSourcesQuery([baseSource]));
  });
