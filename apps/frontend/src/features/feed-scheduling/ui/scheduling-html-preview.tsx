...
/**
 * Sanitizing mini-renderer for the scheduling-body preview.
 *
 * The backend sanitizes scheduling bodies for Telegram's `parse_mode: HTML` (see
 ...
 * survives untouched.
 */
function tokenizeSchedulingHtml(input: string): Token[] {
  const tokens: Token[] = [];
  let i = 0;
  let text = '';
...
/** Parse a scheduling body into a sanitized React node tree (never raw HTML). */
export function parseSchedulingHtml(body: string): ReactNode[] {
  return buildNodes(tokenizeSchedulingHtml(body), 0).nodes;
}

/** Live, sanitized preview of a scheduling body for the modal. */
export function SchedulingHtmlPreview({
  body,
}: {
  body: string;
}): React.ReactElement {
  return <div className="whitespace-pre-wrap">{parseSchedulingHtml(body)}</div>;
}
