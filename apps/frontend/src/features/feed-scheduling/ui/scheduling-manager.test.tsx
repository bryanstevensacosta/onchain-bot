...
import {
  ...
  isoToUtcInput,
  utcInputToIso,
} from './scheduling-manager';
import type { SchedulingView, MediaLibraryView } from '../api/scheduling-api';

afterEach(cleanup);

function makeAd(overrides: Partial<SchedulingView> = {}): SchedulingView {
  return {
    id: 'scheduling-1',
    name: 'Pump alpha',
    body: 'Something good',
...
const updateSchedulingMock = vi.fn();
const reuseMutMock = vi.fn();
const publishNowMutMock = vi.fn();

vi.mock('@/features/feed-scheduling/model/use-scheduling', () => ({
  useScheduling: vi.fn(),
  useCreateScheduling: vi.fn(),
  useUpdateScheduling: vi.fn(),
  useDeleteScheduling: vi.fn(),
  useUploadSchedulingImage: vi.fn(),
  useUploadSchedulingVideo: vi.fn(),
  useClearSchedulingImage: vi.fn(),
  useMediaLibrary: vi.fn(),
  usePublishSchedulingNow: vi.fn(),
...
import {
  useScheduling,
  useClearSchedulingImage,
  useCreateScheduling,
  useDeleteScheduling,
  useMediaLibrary,
  usePublishSchedulingNow,
  useReuseLibraryImage,
  useReuseLibraryImages,
  useUpdateScheduling,
  useUploadSchedulingImage,
  useUploadSchedulingVideo,
} from '@/features/feed-scheduling/model/use-scheduling';

const mockedUseScheduling = vi.mocked(useScheduling);
const mockedUseCreateAd = vi.mocked(useCreateScheduling);
const mockedUseUpdateAd = vi.mocked(useUpdateScheduling);
const mockedUseDeleteAd = vi.mocked(useDeleteScheduling);
const mockedUseUploadAdImage = vi.mocked(useUploadSchedulingImage);
const mockedUseUploadAdVideo = vi.mocked(useUploadSchedulingVideo);
const mockedUseClearAdImage = vi.mocked(useClearSchedulingImage);
const mockedUseMediaLibrary = vi.mocked(useMediaLibrary);
const mockedUsePublishAdNow = vi.mocked(usePublishSchedulingNow);
...
describe('SchedulingManager', () => {
  ...
  it('renders the scheduling list from the mocked query', () => {
    mockedUseScheduling.mockReturnValue({
      data: [
        makeAd({ name: 'Alpha' }),
        makeAd({ id: 'scheduling-2', name: 'Bravo', order: 1 }),
      ],
      isLoading: false,
      error: null,
    } as never);
  ...
  it('shows an error state when the query rejects', () => {
    ...
    render(<SchedulingManager />);
    expect(screen.getByText(/Failed to load scheduling/)).toBeInTheDocument();
  });

  it('shows the empty state when there are no scheduling', () => {
    mockedUseScheduling.mockReturnValue({
      data: [],
      isLoading: false,
      error: null,
    ...
    render(<SchedulingManager />);
    expect(screen.getByText(/No scheduling yet/)).toBeInTheDocument();
  });
  ...
  it('opens the create modal and submits name/body', () => {
    ...
    render(<SchedulingManager />);
    fireEvent.click(screen.getByText('+ Add Scheduling'));
  ...
  it('pre-fills the edit modal with the scheduling buttons', () => {
    mockedUseScheduling.mockReturnValue({
      data: [
        makeAd({
          buttons: [{ text: 'Abrir', url: 'https://example.com' }],
  ...
  it('submits buttons in the edit patch', () => {
    ...
    expect(updateSchedulingMock).toHaveBeenCalledWith(
      {
        id: 'scheduling-1',
  ...
  it('add scheduling modal only closes via the × button (not backdrop or Escape)', () => {
    mockedUseScheduling.mockReturnValue({
      data: [],
      isLoading: false,
      error: null,
    ...
    expect(screen.getByText('Add Scheduling')).toBeInTheDocument();
    ...
    expect(screen.queryByText('Add Scheduling')).not.toBeInTheDocument();
  });

  it('toggles enabled via updateScheduling', () => {
    mockedUseScheduling.mockReturnValue({
  ...
  it('moves a scheduling up by swapping order values with its neighbor', () => {
    mockedUseScheduling.mockReturnValue({
      data: [
        makeAd({ id: 'scheduling-first', name: 'First', order: 0 }),
        makeAd({ id: 'scheduling-second', name: 'Second', order: 1 }),
      ],
      isLoading: false,
      error: null,
    } as never);
  ...
  it('moves a scheduling down by swapping order values with its neighbor', () => {
    mockedUseScheduling.mockReturnValue({
      data: [
        makeAd({ id: 'scheduling-first', name: 'First', order: 0 }),
        makeAd({ id: 'scheduling-second', name: 'Second', order: 1 }),
  ...
  it('disables move buttons at the list edges', () => {
    mockedUseScheduling.mockReturnValue({
      data: [makeAd({ id: 'scheduling-only', name: 'Only', order: 0 })],
      isLoading: false,
      error: null,
    } as never);
    render(<SchedulingManager />);
  ...
  it('pre-fills the edit modal with the scheduling expiry', () => {
    mockedUseScheduling.mockReturnValue({
      data: [
        makeAd({
  ...
  it('submits an empty expiry on edit as null', () => {
    ...
    expect(updateSchedulingMock).toHaveBeenCalledWith(
      {
        id: 'scheduling-1',
        patch: expect.objectContaining({ expiresAt: null }),
      },
      expect.anything(),
    );
  });

  it('shows an expired badge for a scheduling past its expiry', () => {
    mockedUseScheduling.mockReturnValue({
  ...
  it('shows "no limit" when the scheduling never expires', () => {
    mockedUseScheduling.mockReturnValue({
  ...
  it('uploads a selected file via the row file input', () => {
    ...
    const { container } = render(<SchedulingManager />);
  ...
  it('removes the image via clearSchedulingImage after confirmation', () => {
    ...
    expect(clearMut).toHaveBeenCalledWith('scheduling-1');
  ...
  it('sends the scheduling to Telegram now after confirmation', () => {
    mockedUseScheduling.mockReturnValue({
      data: [makeAd()],
      isLoading: false,
      error: null,
    ...
    expect(publishNowMutMock).toHaveBeenCalledWith('scheduling-1');
    confirmSpy.mockRestore();
  });

  it('does not send the scheduling when the confirmation is dismissed', () => {
    mockedUseScheduling.mockReturnValue({
      data: [makeAd()],
      isLoading: false,
      error: null,
  ...
  it('shows inline confirmation only on the published row', () => {
    mockedUseScheduling.mockReturnValue({
      data: [
        makeAd({ id: 'scheduling-1', name: 'Alpha', order: 0 }),
        makeAd({ id: 'scheduling-2', name: 'Bravo', order: 1 }),
      ],
      isLoading: false,
      error: null,
  ...
  it('renders create modal; image controls appear for photo', () => {
    ...
    const modalCard = screen
      .getByText('Add Scheduling')
      .closest('div.bg-slate-900') as HTMLElement;
    expect(modalCard).not.toBeNull();

    expect(
      within(modalCard).queryByTestId('scheduling-image-file-input'),
    ).toBeNull();
    expect(
    ...
    expect(
      within(modalCard).getByTestId('scheduling-image-file-input'),
    ).toBeInTheDocument();
    expect(
      within(modalCard).getByRole('button', { name: /Toggle library picker/i }),
    ).toBeInTheDocument();
    expect(screen.queryByAltText(/current scheduling image/i)).toBeNull();
  });
  ...
  it('offers Reuse and Upload in both image states, Remove only with an image', () => {
    ...
    const { rerender } = render(<SchedulingManager />);
  ...
  it('creates a scheduling and uploads the staged file', () => {
    mockedUseScheduling.mockReturnValue({
      data: [],
      isLoading: false,
      error: null,
    ...
    const createMut = vi.fn(
      (
        _body: unknown,
        opts?: { onSuccess?: (created: SchedulingView) => void },
      ) => {
        opts?.onSuccess?.(makeAd({ id: 'new-scheduling' }));
      },
    );
    mockedUseCreateAd.mockReturnValue({
      isPending: false,
    ...
    expect(uploadMut).toHaveBeenCalledWith(
      { schedulingId: 'new-scheduling', file },
      expect.anything(),
    );
  });

  it('creates a scheduling and reuses a library image', () => {
    mockedUseScheduling.mockReturnValue({
      data: [],
      isLoading: false,
      error: null,
  ...
  it('edits a scheduling and replaces its image via upload', () => {
    mockedUseScheduling.mockReturnValue({
      data: [makeAd({ imageMediaId: 'media-1', format: 'photo' })],
      isLoading: false,
      error: null,
    ...
    updateSchedulingMock.mockImplementation(
      (
        _args: unknown,
        opts?: { onSuccess?: (updated: SchedulingView) => void },
      ) => {
        opts?.onSuccess?.(
          makeAd({
            id: 'scheduling-1',
    ...
    const updateCall = updateSchedulingMock.mock.calls.find(
      (call) =>
        (call[0] as { id: string }).id === 'scheduling-1' &&
        (call[0] as { patch: { image?: unknown } }).patch.image === undefined,
    );
    expect(updateCall).toBeDefined();
    expect(uploadMut).toHaveBeenCalledWith(
      { schedulingId: 'scheduling-1', file },
      expect.anything(),
    );
  });
  ...
  it('shows format-specific controls when switching the format selector', () => {
    ...
    expect(
      within(modalCard).queryByTestId('scheduling-video-file-input'),
    ).toBeNull();
    ...
    expect(
      within(modalCard).getByTestId('scheduling-video-file-input'),
    ).toBeInTheDocument();
    expect(
      within(modalCard).queryByTestId('scheduling-image-file-input'),
    ).toBeNull();
  ...
  it('renders no image controls for a text scheduling row', () => {
    mockedUseScheduling.mockReturnValue({
      data: [makeAd()],
      isLoading: false,
      error: null,
  ...
  it('creates a video scheduling via upload then patches the format in order', () => {
    ...
    const uploadVideoMut = vi.fn(
      (
        ...
        opts?.onSuccess?.(
          makeAd({ id: 'new-scheduling', videoMediaId: 'media-v' }),
        );
      },
    );
    mockedUseUploadAdVideo.mockReturnValue({
    ...
    updateSchedulingMock.mockImplementation(
      (
        ...
        opts?.onSuccess?.(makeAd({ id: 'new-scheduling', format: 'video' }));
      },
    );
    ...
    expect(updateSchedulingMock).toHaveBeenCalledWith(
      { id: 'new-scheduling', patch: { format: 'video' } },
      expect.anything(),
    );
    expect(callOrder).toEqual(['create', 'uploadVideo', 'patchFormat']);
  });

  it('creates an album scheduling reusing library images then patches the format in order', () => {
    mockedUseScheduling.mockReturnValue({
      data: [],
      isLoading: false,
      error: null,
    ...
    const reuseImagesMut = vi.fn(
      (
        ...
        opts?.onSuccess?.(
          makeAd({ id: 'new-scheduling', albumMediaIds: ['lib-1'] }),
        );
      },
    );
    mockedUseReuseLibraryImages.mockReturnValue({
    ...
    updateSchedulingMock.mockImplementation(
      (
        ...
        callOrder.push('patchFormat');
        opts?.onSuccess?.(makeAd({ id: 'new-scheduling', format: 'album' }));
      },
    );
    ...
    expect(reuseImagesMut).toHaveBeenCalledWith(
      { schedulingId: 'new-scheduling', libraryMediaIds: ['lib-1'] },
      expect.anything(),
    );
    expect(updateSchedulingMock).toHaveBeenCalledWith(
      { id: 'new-scheduling', patch: { format: 'album' } },
      expect.anything(),
    );
  ...
  it('renders the body preview with bold formatting', () => {
    ...
    const preview = screen.getByLabelText('Scheduling preview');
    const bold = within(preview).getByText('hola');
    expect(bold.tagName).toBe('B');
    expect(bold).toHaveClass('font-semibold');
  });

  it('locks the format selector when editing a scheduling with a video', () => {
    mockedUseScheduling.mockReturnValue({
    ...
    expect(
      within(modalCard).getByText(
        'Formato bloqueado: el scheduling ya tiene media.',
      ),
    ).toBeInTheDocument();
    expect(
