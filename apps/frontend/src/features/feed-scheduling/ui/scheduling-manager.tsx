...
import {
  ...
  schedulingVideoUrl,
  libraryImageUrl,
  type SchedulingFormat,
  type SchedulingView,
} from '@/features/feed-scheduling/api/scheduling-api';
import { SchedulingButtonPreview } from './scheduling-button-preview';
import { SchedulingHtmlPreview } from './scheduling-html-preview';

/**
 * Modal submit payload: unlike `CreateSchedulingBody`, `expiresAt` is ALWAYS present
 * (`null` = explicit CLEAR, Metis R6.1). `handleCreateSubmit` maps `null` →
 * omitted for the create API; edit passes it through to `UpdateSchedulingBody`.
 * `format` is the target format: the handlers create/update WITHOUT format
 * (backend `Scheduling.create()` defaults to `text` and `validateInvariants()` rejects
 * a format patch with no media), then run the media mutation, then PATCH
 * `{ format }` once the media exists.
 */
export interface SchedulingModalSubmitBody {
  name: string;
  body: string;
...
interface AdModalProps {
  ...
  initialFormat: SchedulingFormat;
  ...
  onSubmit: (body: SchedulingModalSubmitBody) => void;
  pending: boolean;
...
function AdModal({
  ...
  const [format, setFormat] = useState<SchedulingFormat>(initialFormat);
  ...
  // Once a scheduling has media, its format is fixed (backend invariant:
  // `Scheduling.validateInvariants()`). Editing such a scheduling locks the selector.
  const formatLocked =
  ...
  // scheduling instead of stale values from a previous mount.
  useEffect(() => {
    if (isOpen) {
      setName(initialName);
      setBody(initialBody);
  ...
  function handleFormatChange(next: SchedulingFormat) {
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div>
          ...
          {/* Formatting toolbar renders for all formats: the body is published
              ...
              sendMediaGroup in bot-api-feed-publisher.adapter.ts). Note
          ...
          <textarea
            ...
            placeholder="Scheduling copy sent to the output channel"
            rows={3}
            className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-1.5 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500"
            disabled={pending}
          />
        ...
        <div>
          ...
          >
            <SchedulingHtmlPreview body={body} />
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label
              htmlFor="scheduling-expires-at"
              className="block text-xs uppercase text-slate-500 mb-1"
            >
              Expires at (UTC)
            </label>
            <input
              id="scheduling-expires-at"
              type="datetime-local"
              value={expiresAt}
          ...
          <div>
            <label
              htmlFor="scheduling-expiration-action"
              className="block text-xs uppercase text-slate-500 mb-1"
            >
              On expiry
            </label>
            <select
              id="scheduling-expiration-action"
        ...
        {format === 'photo' && (
          <div>
            ...
            <div className="flex flex-col gap-2">
              {initialImageMediaId !== null && pendingImage === null && (
                <img
                  src={schedulingImageUrl(initialImageMediaId)}
                  alt="Current scheduling image"
                  aria-label="Current scheduling image"
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  type="file"
                  accept="image/*"
                  aria-label="Scheduling image file"
                  data-testid="scheduling-image-file-input"
                  className="hidden"
        ...
        {format === 'video' && (
          <div>
            ...
            <div className="flex flex-col gap-2">
              {initialVideoMediaId !== null && pendingVideo === null && (
                <video
                  src={schedulingVideoUrl(initialVideoMediaId)}
                  controls
                  aria-label="Current scheduling video"
              ...
              {pendingVideo !== null && videoPreviewUrl !== null && (
                <video
                  src={videoPreviewUrl}
                  controls
                  aria-label="Scheduling video preview"
                  className="h-24 w-40 rounded border border-slate-700 bg-black"
                />
              )}
              <input
                ...
                type="file"
                accept="video/mp4"
                aria-label="Scheduling video file"
                data-testid="scheduling-video-file-input"
                className="hidden"
 ...
 * time (no ticking interval — freshness comes from the useScheduling poll).
...
function AdImageCell({
  ...
  const uploadMut = useUploadSchedulingImage();
  const clearMut = useClearSchedulingImage();
  const libraryQuery = useMediaLibrary();
  ...
  function handleFileChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    e.target.value = '';
    if (!file) return;
    uploadMut.mutate({ schedulingId: item.id, file });
  }

  function handleRemove() {
    if (typeof window !== 'undefined') {
      const ok = window.confirm(`Remove image for scheduling "${item.name}"?`);
      if (!ok) return;
    }
    ...
    <div className="flex flex-col items-start gap-1">
      <div className="inline-flex items-center gap-2">
        {item.imageMediaId !== null && (
          <>
            <img
              src={schedulingImageUrl(item.imageMediaId)}
              alt={`${item.name} image`}
...
export function SchedulingManager(): React.ReactElement {
  const { data, isLoading, error } = useScheduling();
  const createMut = useCreateScheduling();
  const updateMut = useUpdateScheduling();
  const deleteMut = useDeleteScheduling();
  const uploadMut = useUploadSchedulingImage();
  const reuseMut = useReuseLibraryImage();
  const uploadVideoMut = useUploadSchedulingVideo();
  const reuseImagesMut = useReuseLibraryImages();
  const publishNowMut = usePublishSchedulingNow();

  const [createModalOpen, setCreateModalOpen] = useState(false);
  const [editModalOpen, setEditModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<SchedulingView | null>(null);
  ...
  function handleOpenEdit(item: SchedulingView) {
    setEditingItem(item);
  ...
  function handleCreateSubmit(body: SchedulingModalSubmitBody) {
    ...
    createMut.mutate(
      ...
      {
        onSuccess: (created: SchedulingView) => {
          ...
          if (image) {
            if (image.kind === 'upload') {
              uploadMut.mutate(
                { schedulingId: created.id, file: image.file },
              ...
              reuseMut.mutate(
                {
                  schedulingId: created.id,
                  libraryMediaId: image.libraryMediaId,
                },
                { onSuccess: () => finish(), onError: onMediaError },
              );
            }
            ...
            uploadVideoMut.mutate(
              { schedulingId: created.id, file: video.file },
              { onSuccess: () => finish(), onError: onMediaError },
            );
          } else if (albumMediaIds && albumMediaIds.length > 0) {
            reuseImagesMut.mutate(
              { schedulingId: created.id, libraryMediaIds: albumMediaIds },
              { onSuccess: () => finish(), onError: onMediaError },
            );
          } else {
            finish();
  ...
  function handleEditSubmit(body: SchedulingModalSubmitBody) {
      ...
      { id: editingAdId, patch: metadata },
      {
        onSuccess: () => {
          ...
          if (image) {
            if (image.kind === 'upload') {
              uploadMut.mutate(
                { schedulingId: editingAdId, file: image.file },
                { onSuccess: () => finish(), onError: onMediaError },
              );
            } else {
              reuseMut.mutate(
            ...
            uploadVideoMut.mutate(
              { schedulingId: editingAdId, file: video.file },
              { onSuccess: () => finish(), onError: onMediaError },
            );
          } else if (albumMediaIds && albumMediaIds.length > 0) {
            reuseImagesMut.mutate(
              { schedulingId: editingAdId, libraryMediaIds: albumMediaIds },
              { onSuccess: () => finish(), onError: onMediaError },
            );
          } else {
            finish();
  ...
  function handleToggle(item: SchedulingView) {
    updateMut.mutate({ id: item.id, patch: { enabled: !item.enabled } });
  }

  function handleMove(item: SchedulingView, dir: -1 | 1) {
    const index = scheduling.findIndex((a) => a.id === item.id);
    const target = scheduling[index + dir];
    if (!target) return;
    // Swap semantics: exchange order values with the adjacent scheduling so no
  ...
  function handleDelete(item: SchedulingView) {
    if (typeof window !== 'undefined') {
      const ok = window.confirm(`Delete scheduling "${item.name}"?`);
      if (!ok) return;
    }
    deleteMut.mutate(item.id);
  }

  function handlePublishNow(item: SchedulingView) {
    if (typeof window !== 'undefined') {
      const ok = window.confirm(
        `Send scheduling "${item.name}" to Telegram now?`,
      );
      if (!ok) return;
    }
   ...
   */
  function renderPublishFeedback(item: SchedulingView) {
    ...
    <Card>
      ...
      {createMut.error && (
        <div className="mb-3 text-sm text-red-400">
          Failed to add scheduling: {String(createMut.error)}
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="text-sm text-red-400">
          Failed to load scheduling: {String(error)}
        </div>
      ) : scheduling.length === 0 ? (
        <div className="text-sm text-slate-500">
          No scheduling yet. Add one above to start rotating scheduled content.
        </div>
      ) : (
                  ...
                  <td className="py-2 pr-3">
                    <div className="inline-flex items-center gap-1">
                      ...
                      <button
                        ...
                        onClick={() => handleMove(item, 1)}
                        disabled={
                          index === scheduling.length - 1 || updateMut.isPending
                        }
                  ...
                  <td className="py-2 pr-3">
                    {item.format === 'photo' && (
                      <AdImageCell
                        ...
                        externalError={
                          lastImageError !== null &&
                          item.id === lastImageError.schedulingId
