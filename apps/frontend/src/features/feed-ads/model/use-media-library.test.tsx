...
import {
  ...
  reuseLibraryImage,
  type AdView,
} from '@/features/feed-ads/api/ads-api';
...
vi.mock('@/features/feed-ads/api/ads-api', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@/features/feed-ads/api/ads-api')>();
  return {
    ...actual,
...
describe('useMediaLibrary', () => {
  it('mounts a query with key [feed-ads, media-library], 5s staleTime and 10s refetchInterval', async () => {
    ...
    expect(mockedUseQuery).toHaveBeenCalledWith(
      expect.objectContaining({
        queryKey: ['feed-ads', 'media-library'],
        staleTime: 5_000,
