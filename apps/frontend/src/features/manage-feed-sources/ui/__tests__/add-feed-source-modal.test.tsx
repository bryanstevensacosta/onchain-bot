...
vi.mock('../../model/use-add-feed-source', () => ({
  useAddFeedSource: vi.fn(),
}));

import { useAddFeedSource } from '../../model/use-add-feed-source';
import { AddFeedSourceModal } from '../add-feed-source-modal';
...
function mockHookReturn(
  ...
  (useAddFeedSource as unknown as ReturnType<typeof vi.fn>).mockReturnValue({
...
describe('AddFeedSourceModal', () => {
  ...
  it('renders nothing when isOpen=false', () => {
    renderWithClient(<AddFeedSourceModal isOpen={false} onClose={() => {}} />);
    expect(screen.queryByText('Add Source')).not.toBeInTheDocument();
  });

  it('renders the modal title when isOpen=true', () => {
    renderWithClient(<AddFeedSourceModal isOpen={true} onClose={() => {}} />);
    expect(
  ...
  it('submits the trimmed channelId via mutateAsync and closes on success', async () => {
    ...
    const onClose = vi.fn();
    renderWithClient(<AddFeedSourceModal isOpen={true} onClose={onClose} />);
    fireEvent.change(screen.getByLabelText('Telegram Channel ID'), {
      target: { value: '  -1001234567890  ' },
    });
