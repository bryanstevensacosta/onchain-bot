...
export function KeywordsManager(): React.ReactElement {
  ...
  }, [filteredKeywords.length, page, totalPages]);
        ...
        <>
          ...
          {filteredKeywords.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              ...
              {filteredKeywords.length > PAGE_SIZE && (
                <div className="flex items-center justify-between pt-3 text-xs text-slate-500">
                  <span>
                    {page * PAGE_SIZE + 1}–
                    {Math.min((page + 1) * PAGE_SIZE, filteredKeywords.length)}{' '}
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {Array.from(
                      {
                        length: Math.ceil(filteredKeywords.length / PAGE_SIZE),
                      },
                      (_, i) => (
                        <button
                          key={i}
                    ...
                    <button
                      type="button"
                      disabled={
                        (page + 1) * PAGE_SIZE >= filteredKeywords.length
                      }
                      onClick={() => setPage(page + 1)}
                      className="px-2 py-1 rounded bg-slate-800 border border-slate-700 text-slate-300 hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed"
                    >
