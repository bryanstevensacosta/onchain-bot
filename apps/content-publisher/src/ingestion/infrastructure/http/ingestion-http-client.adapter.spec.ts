...
describe('IngestionHttpClientAdapter', () => {
  it('lists sources through ?type=crypto-news', async () => {
    ...
    await adapter.listCryptoNewsSources();
  ...
  it('fetches recent messages through ?type=crypto-news', async () => {
    ...
    await adapter.fetchRecentCryptoNewsMessages(10);
    expect(seen).toHaveLength(1);
  ...
  it('degrades to [] on transport errors', async () => {
    ...
    await expect(adapter.listCryptoNewsSources()).resolves.toEqual([]);
    await expect(
      adapter.fetchRecentCryptoNewsMessages(10),
    ).resolves.toEqual([]);
