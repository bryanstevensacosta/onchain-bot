import type { CryptoNewsSource } from '../../../domain/ports/ingestion-client.port';

/**
 * Raw crypto-news source row as served by
 * `GET /api/feed/sources?type=crypto-news`.
 * Unknown extra fields are tolerated; only the typed subset is kept.
 */
export interface RawCryptoNewsSourceDto {
  channelId?: unknown;
  channel_id?: unknown;
  peerId?: unknown;
  title?: unknown;
...
export function toCryptoNewsSource(
  raw: RawCryptoNewsSourceDto,
): CryptoNewsSource | null {
  const channelId =
