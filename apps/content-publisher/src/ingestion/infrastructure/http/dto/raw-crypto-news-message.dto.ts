...
/**
 * Raw crypto-news message row as served by
 * `GET /api/feed/messages?type=crypto-news`.
 */
export interface RawCryptoNewsMessageDto {
  channelId?: unknown;
  channel_id?: unknown;
  peerId?: unknown;
  messageId?: unknown;
...
function pickChannelId(raw: RawCryptoNewsMessageDto): string | null {
  const v =
...
function pickMessageId(raw: RawCryptoNewsMessageDto): number | null {
  return (
    asNumber(raw.messageId) ?? asNumber(raw.message_id) ?? asNumber(raw.id)
  );
}

function pickOccurredAt(raw: RawCryptoNewsMessageDto): string {
  const v =
...
/**
 * True when the payload carries the crypto-news marker. Anything else
 * (missing data, missing marker, other marker) is NOT a crypto-news frame.
 */
export function isCryptoNewsData(data: unknown): boolean {
...
/** True for a top-level `message:telegram` frame whose `data` is crypto-news-marked. */
export function isCryptoNewsFrame(frame: unknown): boolean {
  ...
  return isCryptoNewsData(record.data);
}

export function toCryptoNewsMessage(
  raw: RawCryptoNewsMessageDto,
): CryptoNewsIngestedMessage | null {
