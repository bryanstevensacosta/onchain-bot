...
import type { CryptoNewsFeedMedia } from '../../domain/crypto-news-feed-message';
...
export interface FilteredCryptoNewsMessage extends CryptoNewsFeedMessage {
 ...
 *
 * Stateless and side-effect free (no I/O): FilteredCryptoNewsService owns
...
export class MatchingEvaluator {
  ...
  public mergeAlbumGroups(
    matched: ReadonlyArray<FilteredCryptoNewsMessage>,
    rawBatch: ReadonlyArray<CryptoNewsFeedMessage>,
  ): FilteredCryptoNewsMessage[] {
    ...
    for (const m of matched) {
      ...
      const merged: CryptoNewsFeedMedia[] = [];
