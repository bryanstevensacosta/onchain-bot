...
/**
 * Hardcoded registry of 8 trade buttons (moved from backend
 * chain-dexter-bot `infrastructure/telegram/trade-button-registry.ts`
 * — config import re-pointed at DexterBotConfigService).
 *
 * NOTE: URLs are dexter-branded. Affiliate tags use "dexter" as a
 * placeholder — replace with a real affiliate tag via env var when
 * available.
 */
@Injectable()
export class TradeButtonRegistry {
  ...
  public constructor(private readonly botConfig: DexterBotConfigService) {
  ...
  private buildRegistry(): ReadonlyMap<TradeButtonCode, TradeButton> {
    ...
    map.set('PHO', {
      ...
      buildUrl: (address) =>
        `https://photon-sol.tinyastro.io/@dexter?token=${address}`,
    });
    ...
    map.set('TRO', {
      ...
      chains: new Set<ChainId>(['solana']),
      buildUrl: (address) =>
        `https://t.me/solana_trojanbot?start=r-dexter-${address}`,
    });
    ...
    map.set('TRT', {
      ...
      kind: 'trade',
      chains: new Set<ChainId>(['solana']),
      buildUrl: (address) => `https://trojan.com/@dexter?token=${address}`,
    });
    ...
    map.set('MAE', {
      ...
      buildUrl: (address) =>
        `https://t.me/maestro?start=r-dexter&token=${address}`,
    });
    ...
    map.set('BAN', {
      ...
      buildUrl: (address) =>
        `https://t.me/BananaGunSniper_bot?start=ref_dexter-${address}`,
    });
