...
/**
 * AES-256-GCM field encryption for bot tokens.
 *
 * Key resolution: 64-hex `ENCRYPTION_KEY` (`openssl rand -hex 32`,
 * DISTINCT per env) decodes to 32 bytes directly; any other non-empty
 * shape is hashed with SHA-256 (dev convenience, never production).
 * Empty key -> fail-closed `DomainError` (Tier-1 validation enforces it).
 *
