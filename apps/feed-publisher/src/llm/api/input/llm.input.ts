...
const CONTENT_TYPES = ['crypto-news', 'threads', 'global'] as const;
...
export class CreatePromptTemplateDto {
  ...
  @ApiPropertyOptional({
    description: "Owning content type ('global' = reusable catalog)",
    enum: CONTENT_TYPES,
  })
  @IsOptional()
  @IsIn(CONTENT_TYPES)
  ...
  @ApiProperty({ description: 'Gateway model id', example: 'gpt-4o-mini' })
  @IsString()
  @Length(1, 200)
  public model!: string;
...
export class UpdateLlmConfigDto {
  ...
  /**
   * NOT owned here: the single source of truth is `MatchingConfig`
   * (`PATCH feed-publisher/matching/config`). Declared so the global
