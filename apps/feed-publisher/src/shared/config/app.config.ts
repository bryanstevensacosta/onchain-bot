...
export function buildAppConfig(
  ...
  return {
    port: parseInt(env.FEED_PUBLISHER_PORT ?? '3040', 10),
...
export class ConfigValidationError extends Error {
  constructor(public readonly issues: ConfigValidationIssue[]) {
    super(
      `Invalid feed-publisher config: ${issues
...
/**
 * Tier-1 validation for feed-publisher config.
