 ...
 */
@Injectable()
export class IngestionHttpClientAdapter extends FeedIngestionClientPort {
  ...
  override async listFeedSources(): Promise<FeedSource[]> {
    ...
    for (const row of rows) {
      const mapped = toFeedSource(row as RawFeedSourceDto);
      if (mapped) {
        out.push(mapped);
      }
    }
  ...
  override async fetchRecentFeedMessages(
    limit: number,
    channelId?: string,
    ...
    const out: FeedIngestedMessage[] = [];
    for (const row of rows) {
      const mapped = toFeedMessage(row as RawFeedMessageDto);
      if (mapped) {
        out.push(mapped);
      }
    }
