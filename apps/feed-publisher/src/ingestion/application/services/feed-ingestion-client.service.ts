  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  constructor(
    ...
    private readonly port: FeedIngestionClientPort,
    private readonly handler: ProcessFeedMessageHandler = new ProcessFeedMessageHandler(),
  ) {
  ...
  /**
   * Accepts one realtime SSE frame. Returns true when a new feed
   * row was recorded; malformed or non-feed frames are ignored
   * (no throw).
   */
  ...
  /**
   * Reconnect catch-up by cursor: fetch recent feed rows and feed
  ...
  async catchUpAfterReconnect(limit = CATCH_UP_LIMIT): Promise<number> {
    let rows: FeedIngestedMessage[];
    try {
      rows = await this.port.fetchRecentFeedMessages(limit);
    } catch (error) {
      this.logger.warn(
