...
**Per-env Telegram MTProto ingestion → SSE fan-out.** Una sesión MTProto POR instancia/env — dev local, staging ingestion y prod corren la misma imagen, cada una con SU triple (jamás compartida). Puerto **3031** dentro del container (hosts: dev `:3031`, staging `:3033`, prod `:3032`). NestJS 11 + TypeScript 5.7 + `telegram` (GramJS) + Redis + Postgres (TypeORM) + Pino + Prometheus.
...
│       ├── builders/ (feed-path-builder.ts + .spec.ts)
...
└── telegram/
    ...
    ├── feed/ (feed.module.ts — seeder ELIMINADO, 100% DB-driven)
    │   ├── seeds/feed.seed.ts + seeders/feed.seeder.ts (legado)
    │   └── infrastructure/persistence/typeorm/ (entities/feed-source.entity.ts, repositories/feed-source.repository.ts)
    └── shared/
...
| Módulo                                                       | Provee                                                                                                                                                                                                                                                                                                                                                                                                  | Notas                                                                                                                                                                                                        |
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `SharedModule` (`telegram/shared`, `@Global`)                | RedisService, IngestionSafetyConfig, BackendChannelProviderService, FeedSourceRepository, TelegramClientManager, `TelegramListenerPort`→`TelegramMtprotoListenerAdapter`, DeduplicationService, MessagePersistenceCoordinator, LastSeenManager, FloodWaitHandler/Counter, SleepWindowService, TelegramPeerResolver, MessageQueue, MediaDownloaderService, **TelegramMediaExtractorService** (Phase 5.2) | Evita circulares con Kol/Feed/Media. **Phase 5.2 (2026-09)**: TelegramMediaExtractorService encapsula media extraction + download logic                                                                      |
| `TelegramModule`                                             | `DebugTelegramController`; `onModuleInit()` + `refreshChannels()` + `startListening()` + `scheduleChannelRefresh()` (5 min)                                                                                                                                                                                                                                                                             | Sin canales → warn + no escucha; restart listener si cambia la lista. **POST-MIGRATION**: KOLs from backend HTTP, feed from local DB (FeedSourceRepository)                                                  |
...
| `FeedModule`                                                 | vacío (repo vía SharedModule); **POST-MIGRATION (2026-09-05)**: feed.controller + RegisterNewsSourceUseCase                                                                                                                                                                                                                                                                                             | ~~Seeder eliminado; alta vía `POST /api/feed/sources` en backend~~ **DEPRECATED**. NEW: Alta vía `POST /api/feed/sources` en **ingestion-telegram** (sole owner). Backend endpoint deprecated (returns 501). |
...
| `POST /api/feed/sources` | FeedController.addSource | **NEW 2026-09-05**: Creates feed source in ingestion-telegram DB. Body: `{channelId, title?, handle?}`. Returns 201 + source view. Replaces backend endpoint (now deprecated, returns 501). Ingestion-telegram is SOLE OWNER of feed sources. |
...
- **`TelegramMediaExtractorService` (Phase 5.2)** — encapsula media extraction + download. Responsibilities: extract metadata from Telegram media objects (photo/video) + download files via `MediaDownloaderService`. Returns `TelegramMediaAttachment[]` with `filePath`. Used ONLY for feed channels (KOL messages skip media). Extracted from adapter inline logic (~60 LOC reduction).
...
- **TypeORM**: 4 entities registered in `app.module.ts`:
  - `FeedSourceEntity` — sources (**SOLE OWNER** since 2026-09-05, reads+writes)
  - `FeedMessageEntity` — RAW message content (ingested from Telegram)
  - `FeedMessageMediaEntity` — media metadata (URLs to served files)
...
# Staging ingestion (misma imagen, OTRA triple — la vieja cuenta de dev):
...
# (hashes, nunca valores) ANTES del primer boot — ver runbook de staging ingestion:
...
Últimos commits (hasta 2026-09-03): migración seed→DB (`replace seed-based subscription with DB-driven channel lists`, `replace feed seed with DB query`, `remove feed seeder and enforce DB-driven architecture`), fixes de ingestión feed + DB cache, `prevent AUTH_KEY_UNREGISTERED filtering users/bots`, `remove /api prefix from backend endpoint URLs`, limpieza de credenciales reales en templates. Dirección: eliminar listas estáticas y endurecer anti-ban/sesión — coherente con gaps 15/17/24 (quitar, no revivir, el código seed).
...
- `KolSeeder/FeedSeeder` legado: no añadir canales por seeds; alta por API del backend.
