...
| `POST /api/crypto-news/sources`                                 | CryptoNewsController.addSource   | **NEW 2026-09-05**: Creates crypto-news source in ingestion-service DB. Body: `{channelId, title?, handle?}`. Returns 201 + source view. Replaces backend endpoint (now deprecated, returns 501). Ingestion-service is SOLE OWNER of crypto-news sources.                                                                            |
...
- **Crypto-news**: Ingestion-service owns sources → read directly from local `CryptoNewsSourceRepository.findAllActive()`
- **Rationale**: Eliminates circular dependency (ingestion → backend → ingestion), allows ingestion-service to start without backend, reduces network overhead
...
- **NOTE**: `ChannelContentFilterConfigEntity` removed from ingestion-service (was residual artifact from split). Content filters are **SOLELY OWNED by backend** — backend performs CRUD operations and applies filters on-read via `ContentFilterService`. Ingestion-service stores RAW content only (Opción A architecture).
...
- **KOLs**: Backend owns identity → ingestion-service reads via HTTP
- **Crypto-news sources**: **Ingestion-service SOLE OWNER** → reads+writes from OWN DB (`CryptoNewsSourceRepository`)
...
- **Media**: Ingestion-service owns `uploads/` directory
- **Credentials**: MTProto session exclusive to ingestion-service (avoids `AUTH_KEY_DUPLICATED`)
