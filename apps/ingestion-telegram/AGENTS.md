...
MTProto listener — doble vía:
 ...
 → MessagePersistenceCoordinator.route(raw, type): lastSeen.set → transformToPayload → stream.broadcast({type:'message:telegram'})
...
`BackendChannelProviderService`: `fetchActiveKolIds()` → `GET localhost:{BACKEND_PORT}/telegram-kol/identity/kols/active/ids`; ~~`fetchActiveCryptoNewsSourceIds()` → `GET .../crypto-news/sources/active/ids`~~ (**DEPRECATED** — returns `[]`, crypto-news now read from local DB); `fetchAllActiveChannelIds()` combines (multi-backend mode only). Fallo → `[]` (conserva lista previa).

**Architecture post-migration (2026-09-05)**:

- **KOLs**: Backend owns KOL identity → fetched via HTTP from backend DB
- **Crypto-news**: Ingestion-telegram owns sources → read directly from local `CryptoNewsSourceRepository.findAllActive()`
- **Rationale**: Eliminates circular dependency (ingestion → backend → ingestion), allows ingestion-telegram to start without backend, reduces network overhead
...
3. **Dedup sin aplicar**: llamar `deduplicationService.isDuplicate()` en `MessagePersistenceCoordinator.route()` (o en el adapter) para cumplir Invariante 3.
...
1. **Un solo ingestion-telegram en el droplet** (`docker-compose.ingestion.yml` standalone, host `127.0.0.1:3032` → container `3031`); prohibido `ingestion-staging`, segundas redes o segundas DBs de ingestion en el droplet. Backends staging Y prod consumen ESE MISMO ingestion.
2. **Una sola sesión MTProto** (`INGESTION_TELEGRAM_MTPROTO_*` solo en el `.env.production` de ingestion).
3. **Un solo storage de media** (`uploads/crypto-news/media/`, dueño ingestion-telegram).
...
Requisitos del cliente (Req 2.4): reconexión con backoff 1 s→30 s; **sin** `Last-Event-ID`/replay (gap 22) — diseña el consumidor tolerante a huecos. Backends resuelven la URL base por `INGESTION_TELEGRAM_URL`.
...
Notas: `with-ingestion` referencia `../ingestion-telegram/.env.production` — **no existe en el repo** (solo `.env.production.template`); crearlo desde la plantilla en el droplet, nunca commitear (ver comandos en Persistencia). Los e2e contra droplet usan el puerto host **3032**. En red Docker el provider HTTP a `localhost:3030` no resuelve al backend (gap 25).
