...
**Per-env Telegram MTProto ingestion → SSE fan-out.** Una sesión MTProto POR instancia/env — dev local, staging twin y prod corren la misma imagen, cada una con SU triple (jamás compartida). Puerto **3031** dentro del container (hosts: dev `:3031`, twin `:3033`, prod `:3032`). NestJS 11 + TypeScript 5.7 + `telegram` (GramJS) + Redis + Postgres (TypeORM) + Pino + Prometheus.
...
└── telegram/
    ...
    └── shared/
        ...
        ├── application/services/telegram-media-extractor.service.ts (+ .spec.ts)  # **Phase 5.2**
...
| Módulo                                                       | Provee                                                                                                                                                                                                                                                                                                                                                                                                        | Notas                                                                                                                                                                                                                      |
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SharedModule` (`telegram/shared`, `@Global`)                | RedisService, IngestionSafetyConfig, BackendChannelProviderService, CryptoNewsSourceRepository, TelegramClientManager, `TelegramListenerPort`→`TelegramMtprotoListenerAdapter`, DeduplicationService, MessagePersistenceCoordinator, LastSeenManager, FloodWaitHandler/Counter, SleepWindowService, TelegramPeerResolver, MessageQueue, MediaDownloaderService, **TelegramMediaExtractorService** (Phase 5.2) | Evita circulares con Kol/CryptoNews/Media. **Phase 5.2 (2026-09)**: TelegramMediaExtractorService encapsula media extraction + download logic                                                                              |
| `TelegramModule`                                             | `DebugTelegramController`; `onModuleInit()` + `refreshChannels()` + `startListening()` + `scheduleChannelRefresh()` (5 min)                                                                                                                                                                                                                                                                                   | Sin canales → warn + no escucha; restart listener si cambia la lista. **POST-MIGRATION**: KOLs from backend HTTP, crypto-news from local DB (CryptoNewsSourceRepository)                                                   |
...
| `CryptoNewsModule`                                           | vacío (repo vía SharedModule); **POST-MIGRATION (2026-09-05)**: crypto-news.controller + RegisterNewsSourceUseCase                                                                                                                                                                                                                                                                                            | ~~Seeder eliminado; alta vía `POST /api/crypto-news/sources` en backend~~ **DEPRECATED**. NEW: Alta vía `POST /api/crypto-news/sources` en **ingestion-telegram** (sole owner). Backend endpoint deprecated (returns 501). |
...
MTProto listener — doble vía:
 ...
 → subscribe() yield cola → TelegramModule.startListening clasifica kol|crypto-news (pertenencia a newsIds)
  ...
  → SU backend SSE (1:1 por env: dev :3031, twin :3033, prod :3032) consume
```

Notas de path real:

- **Phase 5.2 refactor (2026-09)**: `transformMessage` delegado a `CryptoNewsMessageTransformer` (text extraction 4-source cascade: `message → text → media.caption → fwdFrom.message`). Media download delegado a `TelegramMediaExtractorService` (**solo crypto-news**; KOL nunca descarga). El servicio encapsula: metadata extraction + `MediaDownloaderService.download()` para photos/videos. Adapter ya no contiene lógica inline de media (~60 LOC eliminadas).
...
- `TelegramMtprotoListenerAdapter` — implementa `TelegramListenerPort` (`subscribe`, `resolveChannelMetadata`/`joinChannel` vía `TelegramPeerResolver`, `disconnect`). ⚠️ Su `onModuleInit` chequeaba `cfg?.telegram?.mtprotoApiId/mtprotoApiHash` (keys inexistentes) → **FIXED (Carril 1)**: lee `telegram.apiId/apiHash`, el guard pasa con creds reales y `markAuthorizedIfTrue()` se invoca. `filterChannels()` es código muerto (se pollean todos los peers, incluidos bots). ~~Comentario de cabecera desactualizado ("no media download" — sí descarga para crypto-news)~~ **Phase 5.2**: refactorizado para usar `TelegramMediaExtractorService`.
- **`TelegramMediaExtractorService` (Phase 5.2)** — encapsula media extraction + download. Responsibilities: extract metadata from Telegram media objects (photo/video) + download files via `MediaDownloaderService`. Returns `TelegramMediaAttachment[]` with `filePath`. Used ONLY for crypto-news channels (KOL messages skip media). Extracted from adapter inline logic (~60 LOC reduction).
...
18. ~~**Retención de media sin janitor**~~ **RESOLVED 2026-09-08**: `CryptoNewsRetentionCleanupScheduler` consume `INGESTION_CRYPTO_NEWS_MEDIA_RETENTION_HOURS` (default 72) para AMBAS pasadas (media + messages). `floodProtection.threshold24h` sigue sin evaluarse.
    Seguimiento backend-media-ownership 2026-09-19: el backend es growth-zero en `uploads/crypto-news/media/` (`ProcessNextQueuedArticleUseCase` descarga a tmp y borra en `finally`, retry re-descarga vía `GET /api/media/...`); su `queue.controller` hace proxy a este servicio cuando no tiene copia. Tabla completa en `docs/deployment/media-ownership.md`.
...
25. ~~**Provider HTTP frágil**: `BackendChannelProviderService` usa `fetch` global sin timeout contra `localhost:{BACKEND_PORT}` hardcodeado — en Docker eso no resuelve al backend (nombres `onchain-bot-*`), y un backend colgado congela `refreshChannels()` sin límite (solapa intervalos de 5 min).~~ **RESOLVED 2026-09-05**: Crypto-news sources now read from local `CryptoNewsSourceRepository` instead of HTTP. Only KOLs still fetched from backend (backend owns KOL identity). HTTP dependency reduced by 50%, ingestion-telegram can start without backend for crypto-news ingestion.
...
3. **Un solo storage de media por env** (`uploads/crypto-news/media/` — named volume propio del twin; dueño ingestion-telegram de SU env).
...
# Twin staging (misma imagen, OTRA triple — la vieja cuenta de dev):
...
# (hashes, nunca valores) ANTES del primer boot — ver runbook twin:
# docs/deployment/staging-twin-runbook.md
```
...
- **Dockerfile**: build `node:22-alpine` (`npm ci --workspace=... --ignore-scripts`, `HUSKY=0`) → runtime con `dumb-init`, usuario `nodejs`, `uploads/crypto-news/media`, `EXPOSE 3031`, `HEALTHCHECK /api/health` (ver gap 23: siempre 200 por stubs), `CMD node apps/ingestion-telegram/dist/src/main.js`. `ARG IMAGE_REVISION` declarado al FINAL a propósito (única declaración — el merge #250 había duplicado el bloque T2 original; su SHA cambia por push e invalidaría las capas de install/cache si estuviera arriba).
...
Unit co-locados (`*.spec.ts`): `app.module`, `stream.service`, `sse-stream.controller` (stream abierto, sin gate), `media.controller` **(14 tests, Phase 4 restored)**, `health.controller`, `metrics.service`+`controller`, `app.config`, `structured-logger`, `deduplication`, `message-payload`, `ingestion.coordinator.integration`, **`telegram-media-extractor.service` (7 tests, Phase 5.2)**, **`shared/media/*` (45 tests across 5 files, Phase 1)**, **`crypto-news-retention-cleanup.scheduler` (7 tests, split 2026-09-08)**, **avatar (`kol-avatar.controller/service`, `mtproto-avatar-photo.adapter`, `kol-avatar.janitor` — fetch-once/cached/placeholder, cola serializada, exclusión del janitor pineada)**. **Total: 815 tests across 43 suites** (task-10 evidence, pre-Tramo 1; avatar suma 4 specs).
...
Requisitos del cliente (Req 2.4): reconexión con backoff 1 s→30 s; **sin** `Last-Event-ID`/replay (gap 22) — diseña el consumidor tolerante a huecos. Cada backend resuelve la URL de SU ingestion por `INGESTION_TELEGRAM_URL` (dev `:3031`, twin staging `:3033`, prod `:3032`).
...
apps/backend ──~~GET :3030/crypto-news/sources/active/ids~~ ─────→ **DEPRECATED 2026-09-05** (endpoint exists, returns [])
...
ingestion ──reads+writes──→ crypto_news_sources (own Postgres DB, NOT backend DB — **SOLE OWNER since 2026-09-05**)
ingestion ──POST /api/crypto-news/sources ───────────────────────→ creates sources in OWN DB (backend POST deprecated, returns 501)
```

**Architecture post-migration (2026-09-05 — COMPLETED)**:

- **KOLs**: Backend owns identity → ingestion-telegram reads via HTTP
- **Crypto-news sources**: **Ingestion-telegram SOLE OWNER** → reads+writes from OWN DB (`CryptoNewsSourceRepository`)
  ...
  - Backend `CryptoNewsSourceRepository.save()/delete()` → throws error
  - Backend `RegisterNewsSourceUseCase.execute()` → throws error
  - Backend `CryptoNewsSeeder.seed()` → returns early with warning if enabled (should be disabled)
