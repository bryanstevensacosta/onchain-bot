...
4. **Métricas sin alimentar**: inyectar `MetricsService` en adapter/coordinator/stream y actualizar contadores (el README de `src/metrics/` ya documenta cómo). Remanente: el gauge `channelUnionSize` sigue definido (fijado a 0 en init, nadie lo actualiza) — es el cadáver de la union multi-backend eliminada; borrarlo o alimentarlo, no dejarlo en 0.
...
Notas: `with-ingestion` referencia `../ingestion-telegram/.env.production` — **no existe en el repo** (solo `.env.production.template`); crearlo desde la plantilla en el droplet, nunca commitear (ver comandos en Persistencia). Los e2e contra droplet usan el puerto host de SU env (**3032** prod, **3033** twin). `BACKEND_URL` fue eliminada (per-env T4, sin lector); CORS sigue leyendo `BACKEND_STAGING_URL`/`BACKEND_PROD_URL` en `main.ts:73-74`.
