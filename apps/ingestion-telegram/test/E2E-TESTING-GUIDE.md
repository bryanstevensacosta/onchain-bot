...
Per Architectural Invariant 7 (AUTH_KEY_DUPLICATED), attempting to initialize a second MTProto client while the ingestion-telegram is running on the droplet will result in a `406 AUTH_KEY_DUPLICATED` error from Telegram.

## Test Strategy

### ✅ Allowed: Tests that connect to running ingestion-telegram

These tests connect to the ingestion-telegram SSE endpoint on the droplet without initializing MTProto:

```typescript
// CORRECT: Connect to remote ingestion-telegram
...
- **Strategy**: Connect SSE client to droplet ingestion-telegram
...
1. Ingestion-telegram must be running on droplet (144.126.203.139:3032)
...
export INGESTION_TELEGRAM_URL=http://144.126.203.139:3032
...
**Cause**: Test attempted to initialize MTProto while ingestion-telegram on droplet is using the same session.

**Solution**:

1. Stop the test that initialized MTProto
2. Verify ingestion-telegram on droplet is still running: `curl http://144.126.203.139:3032/api/health`
3. If ingestion-telegram crashed, restart: `ssh root@144.126.203.139 "cd /opt/onchain-bot/apps/backend && docker compose -f docker-compose.ingestion.yml restart"`

### SSE Connection Refused

**Symptom**: Tests fail with `ECONNREFUSED 144.126.203.139:3032`

**Cause**: Ingestion-telegram not running on droplet.
