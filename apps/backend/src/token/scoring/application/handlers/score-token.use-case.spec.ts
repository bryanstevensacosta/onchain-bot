...
class FakeReputation extends KolReputationPort {
  ...
  public async getReputation(kolId: string): Promise<KolReputationSummary> {
    const score = this.defaults[kolId] ?? 0.5;
    return await Promise.resolve(KolReputationSummary.create({ kolId, score }));
  }
...
describe('ScoreTokenUseCase', () => {
  ...
  it('scores a healthy multi-channel buzz as STRONG (>=80)', async () => {
    ...
    // tier is now a string (from the VO's `value` getter in the view)
    expect(view.classification).toBe('TOKEN');
    expect(view.avgKolReputation).toBeCloseTo(0.925, 2);
    expect(
  ...
  it('caps SCAM security flag at 5 even if metrics are perfect', async () => {
    ...
    expect(
      view.breakdown.find((b) => b.factor === 'SECURITY_FLAG_CAP'),
    ).toBeDefined();
  });

  it('caps UNKNOWN security flag at 20', async () => {
    const rep = new FakeReputation();
    const useCase = new ScoreTokenUseCase(rep, repo, publisher);
