...
export class RedisAchievementCacheAdapter extends AchievementCachePort {
  private readonly logger = new Logger(RedisAchievementCacheAdapter.name);
  private static readonly KEY_PREFIX = 'achievement:notified:';
  ...
  async addNotifiedThreshold(callId: string, threshold: number): Promise<void> {
    ...
    try {
      ...
      await client.expire(key, RedisAchievementCacheAdapter.TTL_SECONDS);
    } catch (err) {
      this.logger.warn(
  ...
  private keyFor(callId: string): string {
    return `${RedisAchievementCacheAdapter.KEY_PREFIX}${callId}`;
  }
}
