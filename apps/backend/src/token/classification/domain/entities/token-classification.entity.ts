...
import {
  RiskSignal,
  Severity,
} from 'token/classification/domain/value-objects/risk-signal.vo';
import { TokenClassifiedEvent } from 'token/classification/domain/events/token-classified.event';
...
export class TokenClassification extends AggregateRoot<string> {
   ...
   * and confidence recomputation — assumes the DB stored a coherent state.
   */
  public static rehydrate(input: {
    ...
    return new TokenClassification(input.id, {
      chain: input.chain,
  ...
  public get securityFlag(): SecurityFlag {
    return this.state.securityFlag;
  }
  ...
  public emitClassified(): void {
    this.apply(
      new TokenClassifiedEvent({
        ...
        securityFlag: this.state.securityFlag.value,
