...
const CRON_NAME = 'live-milestone-scheduler';
const DEFAULT_BATCH_SIZE = 30;

@Injectable()
export class LiveMilestoneScheduler implements OnApplicationBootstrap {
  private readonly logger = new Logger(LiveMilestoneScheduler.name);
  ...
  onApplicationBootstrap(): void {
    ...
    if (!cfg?.schedulerEnabled) {
      this.logger.log('LiveMilestoneScheduler disabled via config');
      return;
    }
    ...
    job.start();
    this.logger.log(
      `LiveMilestoneScheduler started (cron=${cfg.schedulerCron})`,
    );
  }
