...
function makeAdapter(redis: FakeRedisService): {
  adapter: RedisMilestoneCacheAdapter;
  client: FakeClient;
  redis: FakeRedisService;
} {
  const adapter = new RedisMilestoneCacheAdapter(redis as never);
  return { adapter, client: redis.getClient() as FakeClient, redis };
}

describe('RedisMilestoneCacheAdapter', () => {
  describe('getNotifiedThresholds', () => {
    ...
    it('uses the milestone:notified: key prefix', async () => {
      const client: FakeClient = {
        smembers: jest.fn().mockResolvedValue([]),
        sadd: jest.fn(),
        expire: jest.fn(),
