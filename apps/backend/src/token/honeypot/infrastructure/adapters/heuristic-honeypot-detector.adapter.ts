...
} from 'token/honeypot/domain/ports/honeypot-analyzer.port';
 ...
 */
@Injectable()
export class HeuristicHoneypotDetectorAdapter extends HoneypotAnalyzerPort {
  private readonly logger = new Logger(HeuristicHoneypotDetectorAdapter.name);
  ...
  public async analyze(
    ...
    try {
      const { data } = await axios.get<DexScreenerResponse>(
        `${HeuristicHoneypotDetectorAdapter.ENDPOINT}/${address}`,
        { timeout: 5000 },
      );
