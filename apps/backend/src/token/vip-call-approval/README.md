...
Forma parte de `src/discovery/` y se monta vía `FiltersModule` (`filters.module.ts:23`).

---

## 1. Propósito

Este BC es el **gate final** del pipeline discovery. Toma el score calculado por `scoring` + el contexto del classification, aplica N gates fail-fast, y emite `vip-call.approval.approved` (al BC `publishing`) o `vip-call.approval.rejected` (a ops/dashboards). Es el último punto donde un token "malo" puede ser descartado antes de tocar al usuario final.
...
| Publicación de `vip-call.approval.approved`/`vip-call.approval.rejected` | `application/handlers/apply-filters.use-case.ts:156`    |
...
- **Eventos:** emite `vip-call.approval.approved` (`token-filtered.event.ts:23`) si `verdict=APPROVED`, o `vip-call.approval.rejected` (`token-rejected.event.ts:28`) si `REJECTED`.
...
- `TokenFilteredEvent` (`domain/events/token-filtered.event.ts:7`) — `eventName = 'vip-call.approval.approved'` (`:23`).
- `TokenRejectedEvent` (`domain/events/token-rejected.event.ts:7`) — `eventName = 'vip-call.approval.rejected'` (`:28`); payload incluye `reasons[]`.
...
ApplyFiltersUseCase.execute
        ...
        |
        +--> decision.emit()
        |       APPROVED → TokenFilteredEvent   → 'vip-call.approval.approved'
        |       REJECTED → TokenRejectedEvent    → 'vip-call.approval.rejected'
        |
        +--> FiltersEventPublisher.publishAll
        |
        v
vip-call.approval.approved  -->  Publishing BC
vip-call.approval.rejected   -->  ops/dashboards
```
...
- `application/handlers/apply-filters.use-case.spec.ts` — orquestación con fake `BlacklistPort`; cubre cada uno de los 7 gates (SCORE_TOO_LOW, CLASSIFICATION_BLOCKED, BLACKLISTED, HONEYPOT_SUSPECTED, RISK_WEIGHT_EXCEEDED, INSUFFICIENT_DATA, CHAIN_UNSUPPORTED); verifica que APPROVED se publica como `vip-call.approval.approved` y REJECTED como `vip-call.approval.rejected`.
