...
export class TokenSnapshot extends AggregateRoot<string> {
  ...
  /**
   * Reconstruct an aggregate from persistence. Bypasses validation and
   * primary-pair recomputation — assumes the DB stored a coherent state.
   */
  ...
  public get imageUrls(): ReadonlyArray<string> {
    return this.state.imageUrls;
  }
  public get lockedLiquidityPercent(): number | null {
    return this.state.lockedLiquidityPercent;
  }
  public get burnedPercent(): number | null {
    return this.state.burnedPercent;
  }
  ...
  public emitEnriched(): void {
    this.apply(
      new TokenEnrichedEvent({
        ...
        lockedLiquidityPercent: this.state.lockedLiquidityPercent,
        burnedPercent: this.state.burnedPercent,
