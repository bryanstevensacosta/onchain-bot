...
- **Supported MIME types**: JPEG (`image/jpeg`), PNG (`image/png`), GIF (`image/gif`), WebP (`image/webp`). All others fall back to `application/octet-stream` and `.bin` extension. Detection is magic-byte sniffing on the first bytes of the downloaded buffer, not the Telegram-declared MIME. **Serving** goes through `shared/common/http/media-serving.ts` (`detectMediaMimeType`), which re-sniffs the buffer at request time — so MP4/MOV videos stored as `.bin` + octet-stream are served as `video/mp4` — and supports HTTP Range (`206 Partial Content`) for `<video>` seeking.
- **Photos + videos**: `CryptoNewsMediaType = 'photo' | 'video'`. Videos are downloaded synchronously like photos (same `fileReference` expiry) and served with Range support; stickers, voice notes, and other document attachments are intentionally not modelled per plan scope guardrails.
