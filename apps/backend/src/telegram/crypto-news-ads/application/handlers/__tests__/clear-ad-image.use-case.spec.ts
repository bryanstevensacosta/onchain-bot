import { ClearAdImageUseCase } from '../clear-ad-image.use-case';
...
describe('ClearAdImageUseCase', () => {
  let useCase: ClearAdImageUseCase;
  const adRepo = {
    findById: jest.fn(),
    save: jest.fn(),
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    useCase = new ClearAdImageUseCase(adRepo, adMediaRepo, storage);
  });
  ...
  it('is a no-op returning the ad unchanged when it has no image', async () => {
  ...
  it('removes the file, deletes the media row, and saves the ad with imageMediaId null', async () => {
    ...
    // Ordering: file removed, then row deleted, then ad saved.
