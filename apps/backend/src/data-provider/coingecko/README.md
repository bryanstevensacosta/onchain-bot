...
```typescript
import { CoinGeckoService } from 'apps/market-data/src/provider/infrastructure/coingecko';

// El servicio se inyecta automáticamente (DataProviderModule es @Global)

// 1. Precio de Wrapped SOL en Solana
const info = await coingecko.getTokenContractInfo(
  'solana',
...
const usdt = await coingecko.getTokenContractInfo(
  'ethereum',
