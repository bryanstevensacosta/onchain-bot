...
> ⚠️ Endpoints wallet (v1/wallet/_, wallet/v2/_) y holder (holder/_, token/v1/holder/_) son **solo Solana**.
...
```typescript
import { BirdeyeService } from 'apps/market-data/src/provider/infrastructure/birdeye';
...
async function getRemainingCredits(): Promise<number | null> {
  const response = await birdeye['get']<{ credits: number }>(
    '/utils/v1/credits',
  );
