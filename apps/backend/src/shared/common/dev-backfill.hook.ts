...
export class DevBackfillHook implements OnApplicationBootstrap {
  ...
  public async onApplicationBootstrap(): Promise<void> {
    ...
    try {
      ...
      const ingestion = this.moduleRef.get(StartKolIngestionUseCase, { strict: false });
      const all = await kolRepo.findAll();
      const active = all.filter((k) => k.lifecycleStatus === 'ACTIVE');
      if (active.length === 0) { this.logger.log('No ACTIVE KOLs'); return; }

      this.logger.log(`Backfilling ${active.length}/${all.length} ACTIVE KOLs (limit=5 each)`);
      let total = 0;
      for (const kol of active) {
        try {
          const result = await ingestion.backfillKol(kol.id, 5);
          total += result.total;
          this.logger.log(`  ${kol.id}: +${result.ingested}/${result.total} msgs`);
        } catch (err) {
          this.logger.warn(`  ${kol.id}: backfill failed — ${(err as Error).message}`);
        }
      }
      this.logger.log(`Startup backfill done: ${total} messages`);
    } catch (err) {
      this.logger.warn(`Startup backfill unavailable: ${(err as Error).message}`);
    }
  }
}