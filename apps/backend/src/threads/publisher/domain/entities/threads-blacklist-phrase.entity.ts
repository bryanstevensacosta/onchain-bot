...
import type { ThreadsMatchMode } from './threads-keyword.entity';

const MIN_PHRASE_LENGTH = 1;
const MAX_PHRASE_LENGTH = 200;

interface ThreadsBlacklistPhraseProps {
 ...
 * threads-publisher BC to block incoming messages.
 *
 * Persistence: @Entity({ name: 'threads_blacklist_phrases' }) counterpart lives in
 ...
 */
export class ThreadsBlacklistPhrase extends AggregateRoot<string> {
  private state: ThreadsBlacklistPhraseProps;

  protected constructor(id: string, props: ThreadsBlacklistPhraseProps) {
    super(id);
    this.state = props;
  }
  ...
  public static create(input: {
    ...
    enabled?: boolean;
    createdAt?: Date;
  }): ThreadsBlacklistPhrase {
    if (input.phrase === null || input.phrase === undefined) {
      throw new DomainError(
        ErrorCode.VALIDATION,
        'ThreadsBlacklistPhrase phrase cannot be null/undefined',
      );
    }
    if (typeof input.phrase !== 'string') {
      throw new DomainError(
        ErrorCode.VALIDATION,
        'ThreadsBlacklistPhrase phrase must be a string',
      );
    }
    ...
    if (trimmed.length < MIN_PHRASE_LENGTH) {
      throw new DomainError(
        ErrorCode.VALIDATION,
        'ThreadsBlacklistPhrase phrase cannot be empty',
        { phrase: input.phrase },
      );
    }
    if (trimmed.length > MAX_PHRASE_LENGTH) {
      throw new DomainError(
        ErrorCode.VALIDATION,
        `ThreadsBlacklistPhrase phrase exceeds max length ${MAX_PHRASE_LENGTH}`,
        { length: trimmed.length, max: MAX_PHRASE_LENGTH },
      );
    }
    return new ThreadsBlacklistPhrase(input.id ?? crypto.randomUUID(), {
      phrase: trimmed,
  ...
  public static reconstitute(input: {
    ...
    return new ThreadsBlacklistPhrase(input.id, {
      phrase: input.phrase,
