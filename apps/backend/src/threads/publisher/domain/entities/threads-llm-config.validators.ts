...
const requireInteger = (
  ...
  if (raw === null || raw === undefined || !Number.isInteger(raw)) {
    throw new DomainError(
      ...
      `ThreadsLlmConfig.${field} must be an integer in [${min}, ${max}] (got ${String(raw)})`,
      { [field]: raw },
    );
  }
  if ((raw as number) < min || (raw as number) > max) {
    throw new DomainError(
      ErrorCode.VALIDATION,
      `ThreadsLlmConfig.${field} must be an integer in [${min}, ${max}]`,
      { [field]: raw },
    );
  }
  void label;
...
const requirePositive = (raw: unknown, field: string): number => {
  if (
    ...
    throw new DomainError(
      ErrorCode.VALIDATION,
      `ThreadsLlmConfig.${field} must be a positive number`,
      { [field]: raw },
    );
  }
  return raw as number;
...
export const validateDefaultTemplateId = (raw: unknown): string => {
  if (typeof raw !== 'string' || raw.trim().length === 0) {
    throw new DomainError(
      ErrorCode.VALIDATION,
      'ThreadsLlmConfig.defaultTemplateId must be a non-empty string',
    );
  }
  return raw.trim();
};
...
export const validateRandomDelayMinMs = (raw: unknown): number => {
  if (
    ...
    throw new DomainError(
      ErrorCode.VALIDATION,
      `ThreadsLlmConfig.randomDelayMinMs must be >= ${MIN_DELAY_MS}`,
...
export const validateRandomDelayWindow = (
  ...
  if (validatedMax <= validatedMin) {
    throw new DomainError(
      ErrorCode.VALIDATION,
      'ThreadsLlmConfig.randomDelayMaxMs must be greater than randomDelayMinMs',
