import {
  ...
  validateKolSystemConfig,
} from './app.config';

const TIER_1_KEYS = ['ENCRYPTION_KEY', 'DATABASE_URL'] as const;

describe('kol-system config (Tier-1 validation)', () => {
  ...
  it('passes when all Tier-1 keys are non-empty', () => {
    expect(validateKolSystemConfig({ ...process.env, ...validEnv() })).toEqual({
  ...
  it('fails when ENCRYPTION_KEY is whitespace-only', () => {
    expect(() =>
      validateKolSystemConfig({ ...validEnv(), ENCRYPTION_KEY: '   ' }),
