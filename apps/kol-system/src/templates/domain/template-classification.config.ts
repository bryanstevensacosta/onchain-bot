...
/**
 * Classification AS per-template config (Tramo 1, todos 9+10, P6 + G-08).
 *
 * Moved unchanged from `scoring/` in todo 10 — the templates module owns it
 * from here (the old path re-exports this file so scoring specs stay green).
 * There is NO classification table and NO standalone classification BC: it
 * covers the three P6 faces — visible channels (`kolSourceIds`), score
 * display (`minVisibleScore`), and gem filters (score threshold + regex over
...
export class TemplateClassificationConfig {
  ...
  public static create(
    input: TemplateClassificationConfigProps,
