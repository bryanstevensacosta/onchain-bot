 ...
 *
 * Compares legacy backend published output vs kol-system shadow published
...
if (divergence > threshold) {
  console.log('DIVERGENCE OVER THRESHOLD -> NO cutover. Path: freeze flags (KOL_SYSTEM_ENABLED stays shadow-only), diff logs per mentionId, fix kol-system, re-run dual-run. Gate T1 blocks todo 16.');
  process.exit(2);
}
