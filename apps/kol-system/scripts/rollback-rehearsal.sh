...
export KOL_SYSTEM_ENABLED=true
...
# Step 3: kol-system health still answers (read-only GET) when a port is given.
PORT="${1:-3051}"
if curl -s -o /dev/null -w '%{http_code}' --max-time 5 "http://localhost:${PORT}/api/health" | grep -q '200'; then
  echo "[rehearsal] step 3/3: kol-system :${PORT}/api/health reachable (read-only)"
else
  echo "[rehearsal] step 3/3: kol-system :${PORT} not running — rehearsal continues without live probe (env-only)"
fi
