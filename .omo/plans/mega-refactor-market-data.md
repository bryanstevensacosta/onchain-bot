...
- [x] 0. Precondition: Gates T1/T2 C4-bis (deprecation-check + green) + mapeo libs→src (G-16)
     What to do / Must NOT do: Verificar Gates T1/T2 C4-bis en evidencia: (a) deprecation-check — legacy KOL (T1) y feed (T2) con `@deprecated` + JSDoc (nueva ruta + refactor target); (b) suites T1/T2 en verde; staging (48h T1, 7d T2) es validación y NO se exige completo para arrancar (C4-bis.4). Escribir tabla mapeo `libs/*` del spec → `src/*` del tree (providers→`token/infrastructure/providers`, aggregators→`token/application/services`, cache/rate-limiter→módulos, shared-kernel→`shared/`); sellar Variante B como fase posterior con gate (nº consumers o p95). Must NOT arrancar sin deprecation-checks + green T1/T2.
     ...
     Commit: N | — | —
- [x] 1. App setup + shared kernel (Variante A)
...
- [ ] 13. Dexter hexagonal por sub-BCs (follow-up todo 9)
      ...
      QA scenarios: happy sin regresión; failure diff de rutas → revertir. Evidence .omo/evidence/task-13-mega-refactor-market-data.log
      Commit: Y | refactor(dexter-onchain-bot): hexagonal por sub-BCs
...
- [x] 12. Hexagonal global src/ + legacy deprecado (refactor estructural)
      What to do / Must NOT do: TODO `src/` (gateway, chain, address, provider, cache, rate-limiter, snapshot, shared) en `domain/` + `application/` + `infrastructure/`; deprecar con JSDoc TODO legacy interno (nueva ubicación; borrado en cutover todo 8). Sin cambio de comportamiento (suites iguales ±0). Tests + tsc + boot curl.
