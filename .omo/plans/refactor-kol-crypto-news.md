...
- OpenAPI/Swagger en backend e ingestion (`/api/docs`, alcance feed/ingestion/stream/media).
...
- NO tocar ownership DB del split (backend no escribe feed; ingestion no guarda identidad KOL) ni credenciales MTProto ni crear segundas instancias.
...
- Evidence: .omo/evidence/task-<N>-refactor-kol-feed.txt
...
- [x] 1. Documentar arquitectura dual-path (SSE rápido + polling de respaldo)
     What to do: Crear `docs/architecture/crypto-news-dual-path.md` con diagrama de ambos caminos, tabla de las 4 combinaciones (`USE_SSE_CRYPTO_NEWS` × `matchingEnabled`), estrategia de rollback (`USE_SSE_CRYPTO_NEWS=false`), SLO latencia <10s y resumen de reglas dedup (PENDING/PUBLISHED/bloqueante). Añadir JSDoc `PRIMARY PATH` en `ProcessFeedMessageHandler` y `FALLBACK PATH` en `EnqueueMatchingCronScheduler`. Evitar nombrar las clases `IngestionCoordinator` (las renombra T4).
     Must NOT do: cambiar intervalos, FETCH_LIMIT, lógica de matching o cualquier runtime.
     Parallelization: Wave 1 | Blocked by: — | Blocks: —
     References (executor has NO interview context - be exhaustive): `apps/backend/src/telegram/crypto-news-integration/application/scheduling/enqueue-matching-cron.scheduler.ts:43` (intervalo dinámico, FETCH_LIMIT=50), `apps/backend/src/telegram/crypto-news-integration/application/handlers/process-feed-message.handler.ts:46` (dedup→fetch ventana 10→enqueue→logLatency 10_000ms, nunca lanza), `.kiro/specs/refactor-kol-feed/refactor-recommendations.md` §2 (contenido esperado del doc).
     Acceptance criteria (agent-executable): `test -f docs/architecture/feed-dual-path.md && grep -q 'PRIMARY PATH' apps/backend/src/telegram/crypto-news-integration/application/handlers/process-feed-message.handler.ts && grep -q 'FALLBACK PATH' apps/backend/src/telegram/crypto-news-integration/application/scheduling/enqueue-matching-cron.scheduler.ts && npm run test:backend -- enqueue-matching-cron process-feed-message` en verde (sin cambio de comportamiento).
     QA scenarios (name the exact tool + invocation): happy — `Read` del doc + specs relacionadas en verde vía `npm run test:backend -- <suite>`; failure — `git diff --stat -- apps/backend/src` solo muestra líneas de comentario (cero lógica). Evidence .omo/evidence/task-1-refactor-kol-feed.txt
     Commit: Y | docs(feed): document dual-path SSE plus polling architecture

- [x] 2. Referencia de variables de entorno feed + troubleshooting
     ...
     QA scenarios: happy — cada variable del doc existe en código vía `Grep`; failure — variable documentada sin lector en código se marca y elimina. Evidence .omo/evidence/task-2-refactor-kol-feed.txt
     Commit: Y | docs(feed): environment variable reference plus troubleshooting

- [x] 3. Guía de alta de fuentes feed + rollback
     ...
     Parallelization: Wave 1 | Blocked by: — | Blocks: —
     References: `apps/ingestion-telegram/src/telegram/feed` (alta de sources en ingestion), `apps/backend/src/telegram/ingestion/feed/filters` (CRUD filtros), `apps/backend/src/telegram/feed-publisher` (keywords/blacklist controllers), `.kiro/specs/refactor-kol-feed/refactor-recommendations.md` §9.
     ...
     QA scenarios: happy — walkthrough en seco de la guía contra código (cada paso mapea a ruta real); failure — paso sin ruta real se corrige antes de cerrar. Evidence .omo/evidence/task-3-refactor-kol-feed.txt
     Commit: Y | docs(feed): add-source guide with rollback

- [x] 4. Renombrar los dos `IngestionCoordinator` (backend e ingestion)
     What to do: Backend `IngestionCoordinator` → `MessageRoutingService` (enruta KOL vs feed, no persiste); ingestion-telegram `IngestionCoordinator` → `MessagePersistenceCoordinator` (persiste + emite SSE). Renombrar clase + fichero, actualizar todos los imports/providers/specs y comentarios `Per ...` que citen el nombre viejo. Si algún doc creado en Wave 1 nombra coordinators, actualizarlo.
     ...
     QA scenarios: happy — `npm run test:backend -- ingestion-coordinator` y `npm test -- ingestion.coordinator` (ingestion) en verde; failure — importar el nombre viejo falla en compilación (prueba que el rename es total). Evidence .omo/evidence/task-4-refactor-kol-feed.txt
...
- [x] 5. Validación del 2-flag (`llmEnabled` requiere `publishingEnabled`)
     ...
     Parallelization: Wave 2 | Blocked by: — | Blocks: —
     References: `apps/backend/src/telegram/crypto-news-publisher/api/http/llm-config.controller.ts:221` (updateConfig + guards), `apps/backend/src/telegram/crypto-news-publisher/domain/entities/llm-config.entity.ts:55` (flags + singleton id=1), `llm-config.controller.spec.ts` existente, `.kiro/specs/refactor-kol-feed/refactor-recommendations.md` §5.
     ...
     QA scenarios: happy — PATCH `{llmEnabled:true}` con publishing true → 200 vía spec HTTP; failure — con publishing false → 400 y body contiene `publishingEnabled` (assert exacto en spec). Evidence .omo/evidence/task-5-refactor-kol-feed.txt
     Commit: Y | feat(feed-publisher): validate llm plus publishing flag combination
...
- [x] 6. Buzón DLQ: captura automática + reintento manual on-demand
     ...
     References: `apps/backend/src/telegram/crypto-news-integration/application/handlers/process-feed-message.handler.ts:46` (catch a cablear), `apps/backend/src/telegram/crypto-news-publisher/domain/entities/publisher-queue-entry.entity.ts:101` (patrón entity dual), `apps/backend/src/shared/common/persistence/entities.ts` (registro + count), `EnqueueMatchingMessageUseCase` (retry path), `.kiro/specs/refactor-kol-feed/refactor-recommendations.md` §8.
     ...
     QA scenarios: happy — capturar fallo → fila PENDING; retry → encolado + RETRIED; failure — retry id inexistente → 404; `capture()` lanzando → stream SSE intacto (spec con mock que truena). Evidence .omo/evidence/task-6-refactor-kol-feed.txt
     Commit: Y | feat(feed): dead-letter queue with manual retry
...
- [x] 7. Polling adaptativo + heartbeat SSE configurable
     ...
     QA scenarios: happy — simular 3 tandas de 50 → `setTimeout` 10s invocado (fake timers); failure — tick con `isPolling=true` → skip logueado sin fetch. Evidence .omo/evidence/task-7-refactor-kol-feed.txt
     Commit: Y | feat(feed): adaptive polling plus configurable SSE heartbeat
...
- [x] 8. Limpieza inteligente con vigilancia de disco (ingestion)
     ...
     QA scenarios: happy — disco 85% → limpieza + warn; failure — `statfs` truena → logueado y scheduler sobrevive (spec). Evidence .omo/evidence/task-8-refactor-kol-feed.txt
     Commit: Y | feat(ingestion): disk-aware intelligent media cleanup

- [x] 9. OpenAPI/Swagger en backend e ingestion
     What to do: Añadir `@nestjs/swagger` (versión compatible Nest 11) en ambas apps; `DocumentBuilder` en cada `main.ts` sirviendo en `/api/docs` (verificar que no colisiona con `/api/health`); decoradores en controllers/DTOs de feed, ingestion, stream y media (matching config, llm config, queue, filters, keywords/blacklist, sources, stream status, media). Smoke spec/e2e: `GET /api/docs` → 200 en ambas.
     ...
     References: `apps/backend/src/main.ts:85` (bootstrap), `apps/ingestion-telegram/src/main.ts` (bootstrap), controllers feed-publisher/integration/ingestion + `stream.controller.ts` + `media.controller.ts`, `.kiro/specs/refactor-kol-feed/refactor-recommendations.md` §7 (segunda mitad).
     Acceptance criteria: ambas apps arrancan, `GET /api/docs` → 200 en las dos, suites unit completas verdes (cero rutas rotas).
     QA scenarios: happy — docs cargan y listan paths feed; failure — `GET /api/health` sigue 200 (no colisión) + full suites verdes. Evidence .omo/evidence/task-9-refactor-kol-feed.txt
     Commit: Y | docs(api): OpenAPI Swagger for backend plus ingestion

- [x] 10. FKs opacas documentadas + comentarios ES→EN en código
      What to do: JSDoc en `ChannelContentFilterConfigEntity.channel_id` (varchar opaco, sin FK por diseño del split 2026-09-08) y `PublisherQueueEntry` (FK-less, snapshots de contenido) + mini-doc `docs/architecture/feed-schema-ownership.md` (qué vive en cada DB y por qué no hay FKs cruzadas). Traducir a inglés los comentarios españoles en los 3 `.ts` con hits (`ws.gateway.ts:21`, `command-router.service.ts:120`, `feed-publisher.config.ts:93`); READMEs quedan fuera (90% de los hits, alcance desmedido — anotarlo en el doc).
      Must NOT do: cambiar schema, entidades, queries ni tocar READMEs.
      Parallelization: Wave 2 | Blocked by: — | Blocks: —
      References: `ChannelContentFilterConfigEntity`, `PublisherQueueEntry` entity, los 3 `.ts` citados, `.kiro/specs/refactor-kol-feed/refactor-recommendations.md` §4 (primera mitad).
      ...
      QA scenarios: happy — `tsc --noEmit` limpio + specs de filtros/queue verdes; failure — n/a (docs). Evidence .omo/evidence/task-10-refactor-kol-feed.txt
...
- [x] 11. Migración `message_entities` TEXT→JSONB + GIN (ingestion DB)
      ...
      Parallelization: Wave 4 | Blocked by: — | Blocks: —
      References: `FeedMessageEntity` (ingestion), `apps/ingestion-telegram/src/telegram/shared/application/coordinators/ingestion.coordinator.ts` (lectura de entities), `apps/ingestion-telegram/src/shared/common/persistence/data-source.ts` + `migrations/`, baseline `1788844970659-BaselineIngestionSchema`, `.kiro/specs/refactor-kol-feed/refactor-recommendations.md` §4 (segunda mitad).
      ...
      QA scenarios: happy — up/down + `@>` query vía `psql`; failure — fila con `''` → migra a `'[]'` sin abortar (spec o SQL assert). Evidence .omo/evidence/task-11-refactor-kol-feed.txt
...
- Los 11 todos completos con evidencias en `.omo/evidence/task-<N>-refactor-kol-feed.txt`.
