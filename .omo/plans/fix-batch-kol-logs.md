...
- [x] 1. **app.config.ts: treat empty-string env var as missing, fall back to default KOL metadata cache path**
  ...
  2. Create `apps/backend/src/shared/common/config/app.config.spec.ts` (new — breaks new ground; no existing spec for this file; project `shared/README.md` §16 acknowledges the gap).
  ...
  4. Run `cd apps/backend && npx jest shared/common/config/app.config.spec.ts` — confirm RED.
  5. Edit `app.config.ts:307-311` to read:
     ```ts
  ...
  6. Run the spec — confirm GREEN.
  7. Run `cd apps/backend && npm run test:backend` — confirm no existing spec flipped.
  8. Run `cd apps/backend && npx eslint src/shared/common/config/app.config.ts src/shared/common/config/app.config.spec.ts` — confirm zero issues.
  Must NOT do:
...
- [x] 2. **dev-backfill.hook.ts: silence Telethon PeerUser errors, log skipped count once**
  ...
  2. Create `apps/backend/src/shared/common/dev-backfill.hook.spec.ts` (new).
  3. Write a spec that:
  ...
  4. Run `cd apps/backend && npx jest shared/common/dev-backfill.hook.spec.ts` — confirm RED.
  5. Edit `dev-backfill.hook.ts:36-49`:
  ...
  6. Run the spec — confirm GREEN.
  7. Run `cd apps/backend && npm run test:backend` — confirm no existing spec flipped.
  8. Run `cd apps/backend && npx eslint src/shared/common/dev-backfill.hook.ts src/shared/common/dev-backfill.hook.spec.ts` — confirm zero issues.
  Must NOT do:
...
- [x] 3. **contract-address.vo.ts: validate Solana address in `ContractAddress.fromSolana` (mirror `NormalizedAddress.fromSolana`)**
  ...
  2. Create `apps/backend/src/token/identity/contract-address.vo.spec.ts` (new).
  3. Write a spec with 3 cases:
  ...
  4. Run `cd apps/backend && npx jest token/identity/contract-address.vo.spec.ts` — confirm RED.
  5. Edit `apps/backend/src/token/identity/contract-address.vo.ts`:
  ...
  6. Run the spec — confirm GREEN.
  7. Run `cd apps/backend && npm run test:backend` — confirm no existing spec flipped. **CRITICAL**: pay particular attention to `token/intake/extraction/infrastructure/adapters/regex-based-extractor.adapter.spec.ts` (or similar) — the regex adapter only pre-filters by length, so it should not break. Also `token/intake/parsing/api/http/parsing.controller.ts` callers should be unaffected.
  8. Run `cd apps/backend && npx eslint src/token/identity/contract-address.vo.ts src/token/identity/contract-address.vo.spec.ts` — confirm zero issues.
  Must NOT do:
...
- [x] 4. **vip-call-approval-reason.vo.ts: add INVALID_ADDRESS to reject-reason enum**
  ...
  2. Create `apps/backend/src/token/vip-call-approval/domain/value-objects/vip-call-approval-reason.vo.spec.ts` (new).
  3. Write a spec with cases:
  ...
  4. Run `cd apps/backend && npx jest token/vip-call-approval/domain/value-objects/vip-call-approval-reason.vo.spec.ts` — confirm RED.
  5. Edit `vip-call-approval-reason.vo.ts:3-10` to add `'INVALID_ADDRESS'` to the union (place it first for visibility):
     ```ts
     export type VipCallApprovalReasonCode =
       ...
       | 'CHAIN_UNSUPPORTED';
     ```
  6. Edit `vip-call-approval-reason.vo.ts:37-45` (`VALID_CODES` set) to add `'INVALID_ADDRESS'`.
  7. Run the spec — confirm GREEN.
  8. Run `cd apps/backend && npm run test:backend` — confirm no existing spec flipped.
  9. Run `cd apps/backend && npx eslint src/token/vip-call-approval/domain/value-objects/vip-call-approval-reason.vo.ts src/token/vip-call-approval/domain/value-objects/vip-call-approval-reason.vo.spec.ts` — confirm zero issues.
  Must NOT do:
...
- [x] 5. **apply-vip-call-approval.use-case.ts: add INVALID_ADDRESS gate at position 0 for Solana calls**
  ...
  2. Create `apps/backend/src/token/vip-call-approval/application/handlers/apply-vip-call-approval.use-case.spec.ts` (new). Mirror the `makeFoo()` factory pattern at `apps/backend/src/telegram/vip-calls/vip-channel/application/handlers/vip-calls-publish.use-case.spec.ts:6-97`.
  3. Write a spec with cases:
  ...
  4. Run `cd apps/backend && npx jest token/vip-call-approval/application/handlers/apply-vip-call-approval.use-case.spec.ts` — confirm RED.
  5. Edit `apply-vip-call-approval.use-case.ts:71-148`:
  ...
  6. Run the spec — confirm GREEN.
  7. Run `cd apps/backend && npm run test:backend` — confirm no existing spec flipped. Pay particular attention to the 4 sacred `*.bug-exploration.spec.ts` files.
  8. Run `cd apps/backend && npx eslint src/token/vip-call-approval/application/handlers/apply-vip-call-approval.use-case.ts src/token/vip-call-approval/application/handlers/apply-vip-call-approval.use-case.spec.ts` — confirm zero issues.
  Must NOT do:
