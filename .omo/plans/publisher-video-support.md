...
**Why this approach:** The ingestion pipeline (`extractMediaForMessage`) only handled `MessageMediaPhoto`. Videos arrive as `MessageMediaVideo` or `MessageMediaDocument` with video MIME. We add `extractRawVideoAttachment()`, download video bytes with the same gramjs + FloodWait pattern, extend `TelegramMediaAttachment` to support `type: 'video'`, add `'video'` to `FeedMediaType`, and implement `sendVideo` in the publisher adapter.
...
> TL;DR (machine): Large effort, Medium risk. Add `type: 'video'` to `TelegramMediaAttachment`, `FeedMediaType`, `feed-media.entity` DB. Add `extractRawVideoAttachment()` + download in `extractMediaForMessage`. Add `sendVideo` to port + `BotApiFeedPublisherAdapter`. Extend frontend media type union.
...
- `FeedMediaType`: add `'video'` (was only `'photo'`)
- `FeedMessageMediaEntity`: `type` column already accepts `varchar` — no migration needed
...
- `FeedMediaDownloader.saveToDisk`: increase size limit for videos (10MB → 50MB)
- Frontend: `FeedMediaView.type` union reflects `'photo' | 'video'`
...
- `BotApiFeedPublisherAdapter`: implement `sendVideo` using Telegram Bot API `sendVideo` with multipart, `supports_streaming: true`, optional `thumbnail`
...
| W2   | Domain/VO: add `'video'` to `FeedMediaType` + `TelegramMediaAttachment` + `FeedMedia.create()` accepts `'video'` |
| W3   | Publisher: add `sendVideo` to port + BotApiFeedPublisherAdapter                                                  |
...
- [ ] 1. Add `'video'` to `FeedMediaType` + `TelegramMediaAttachment.type` union
     **What to do:**
  - `feed-media.vo.ts:19`: change `export type FeedMediaType = 'photo';` to `export type FeedMediaType = 'photo' | 'video';`
  - `FeedMedia.create()`: already validates `type` via `FeedMediaType` — no change needed (TS checks at compile time)
  - `telegram-listener.port.ts:45`: change `readonly type: 'photo';` to `readonly type: 'photo' | 'video';`
  - `FeedMediaProps.type`: change from `FeedMediaType` (already correct, no change needed)
  - `feed-message-media.entity.ts`: `type` column is `varchar`, no change needed
  - `feed-message-mapper.ts`: `mediaToEntity/ToDomain` already map `type` as-is, no change needed
    **Must NOT do:** Don't add any DB migrations — `type` column is `varchar` and already accepts any string.
    **References:**
  - `apps/backend/src/telegram/ingestion/feed/domain/value-objects/feed-media.vo.ts:19`
  - `apps/backend/src/telegram/ingestion/shared/domain/ports/telegram-listener.port.ts:45`
    ...
    **QA:** Unit test for `FeedMedia.create({ type: 'video', ... })` — should succeed
    **Parallelization:** Wave 1 | Blocks: 2 | Can parallelize with: 3
    **Commit:** Y | `feat(feed): add 'video' to media type unions`
...
- [ ] 2. Add video extraction and download in TelegramMtprotoListenerAdapter
  ...
  - `apps/backend/src/telegram/ingestion/shared/api/mtproto/telegram-mtproto-listener.adapter.ts:420-437` (extractWebpagePreview)
    ...
    **Commit:** Y | `feat(feed): extract and download video media from Telegram messages`

### Wave 3 — Publisher sendVideo

- [ ] 3. Add `sendVideo` to TelegramPublisherPort + BotApiFeedPublisherAdapter
     **What to do:**
  - `telegram-publisher.port.ts`: add `sendVideo(chatId: string, text: string, imagePath: string): Promise<SendResult>` after `sendPhoto`
  - `bot-api-feed-publisher.adapter.ts`: implement `sendVideo`:
  ...
  - `apps/backend/src/telegram/crypto-news-publisher/infrastructure/senders/bot-api-feed-publisher.adapter.ts:119-175` (sendPhoto as pattern)
    **Acceptance:** `cd apps/backend && npx tsc --noEmit` passes clean
    ...
    **Commit:** Y | `feat(feed): implement sendVideo in publisher adapter`
...
- [ ] 4. Update `dispatchToTelegram` to route video entries to `sendVideo`
  ...
  - In `ProcessNextQueuedArticleUseCase.dispatchToTelegram()`:
    ...
    - Currently `PublisherQueueEntry` stores `imagePaths: string[]` but NOT media types. The media types are only in the `FeedMessage` which was already consumed at enqueue.
  ...
  - `apps/backend/src/telegram/crypto-news-publisher/application/handlers/process-next-queued-article.use-case.ts:140-153`
    ...
    **Commit:** Y | `feat(feed): route video paths to sendVideo in dispatch`

### Wave 5: Frontend

- [ ] 5. Update frontend media type and Recent messages display
     **What to do:**
  - `frontend/src/pages/feed/index.tsx`: check `media[index].type` in the lightbox or image display — for `type: 'video'`, render a video player instead of `<img>`. Actually simplest: use `<video>` tag with controls if MIME is video, otherwise `<img>`.
  ...
  - For now, simplest: if `media[idx].mimeType?.startsWith('video/')`, render `<video>` with `controls className="h-auto w-full max-h-56 rounded object-contain bg-slate-900"` instead of `<img>`.
    ...
    **References:**
  - `apps/frontend/src/pages/feed/index.tsx:200-226` (image grid)
    ...
    **Commit:** Y | `feat(feed): display video media in frontend`
...
- Conventional commit format: `feat(feed): <description>`
- No squashing

## Success criteria

1. `FeedMediaType` accepts `'video'`
