...
- [x] 4. Media rename + mudanza uploads/feed/media + reescritura prefijos
     ...
     References: `apps/ingestion-telegram/src/media/application/services/media-downloader.service.ts:44-45`, `apps/ingestion-telegram/src/media/api/http/media.controller.ts:43-67` (glob por {messageId}*{index}._, NO usa file_path para servir), `apps/ingestion-telegram/src/media/infrastructure/crypto-news-path-builder.ts:53-105` (parse abstrato basename/dirname — reusar), specs media con fixtures `crypto-news/media/...`.
