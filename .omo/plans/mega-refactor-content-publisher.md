...
- [x] 12. BCs content-templates + sessions (P33 + P34) — DONE (evidence `.omo/evidence/task-T2-12.log`: 16 suites / 31 tests; dir renamed `src/content-templates/` → `src/template/` 2026-09-25, evidence `.omo/evidence/rename-template-dir.log`: targeted 16/31 + full 143/460 green, `tsc` clean; module/class/file/API names kept)
      ...
      References: .omo/drafts/mega-refactor-tramos.md (P33, P34); .omo/evidence/content-templates-research.md; .kiro/specs/refactor-feed-publisher/11-refactor.md:2516-3560 (v3); apps/kol-calls/src/templates/ (patrón)
...
- [x] 13. API prefix migration crypto-news → feed, dual-serve 3 fases (P41)
      What to do / Must NOT do: (a) Fase 1 backend dual-serve: alias old+new en cada controller (mapping P41 §11: `crypto-news-publisher`→`feed-publisher`, `crypto-news-scheduling`→`feed-scheduling`, `threads-publisher`→`feed-threads-publisher`, `crypto-news/matching`→`feed-matching`, `crypto-news/filters`→`feed-filters`; `ops/backups` KEPT; `crypto-news/sources`→`feed-sources` ONLY ingestion-telegram, backend NO lo adopta). (b) Fase 2 frontend migra a new (`endpoints.ts`, `vite.config.ts` proxies) + nginx BOTH envs en paralelo: staging `:4173` (`nginx.staging.conf`) tras twin `:3033`, luego prod `:80` (`nginx.conf`) tras prod `:3032` con OK explícito; proxy coherence check vite ≡ nginx × env. (c) Fase 3 cutover: retirar old por env. Tests robustos por fase: fase 1 old+new 200; fase 2 Playwright + coherence; fase 3 old 404/new 200. Rollback por fase (fase 1: quitar alias; fase 2: revert frontend; fase 3: re-añadir alias <30min). Must NOT renombrar `ops/backups`; Must NOT `feed-sources` en backend; Must NOT prod sin staging verde + OK.
      Parallelization: Wave 4 | Blocked by: 9 | Blocks: 11 (cutover)
      References: .omo/drafts/mega-refactor-tramos.md §11 (P41 mapping + estrategia); plan central todo 7 (C-UX-01); apps/frontend/vite.config.ts:51,61,67,81,91,95; apps/frontend/nginx.conf:130,154,169,195,207,219,233; apps/frontend/nginx.staging.conf (mismas líneas)
