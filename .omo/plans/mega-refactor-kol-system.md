...
- Nueva app `apps/kol-calls/` (:3050/3051/3052) con módulos efectivos: ingestion, extraction, parsing, normalization (índice de menciones, G-12), enrichment (vía `MarketDataPort` dual, G-17), scoring, templates (sin threads, G-11), approval, publishing (multi-bot, C2), tracking (first-seen, G-14), telegram (KOL bot), shared. Ver tree `.omo/reference/mega-refactor-target-tree.md`.
...
- P25: `apps/kol-calls/AGENTS.md` vivo — creado en todo 21, actualizado al cierre de cada todo.
...
- [x] 2. App setup: esqueleto kol-system + health + compose (Ph1 spec)
     What to do / Must NOT do: `apps/kol-calls/` con `package.json` (NestJS 11, TypeORM, schedule), `nest-cli.json`, `tsconfig.json`, `src/main.ts` (:3050, ValidationPipe), `src/app.module.ts` (14 imports stub), `.env.example` (`KOL_SYSTEM_ENABLED`, `TEMPLATE_ORCHESTRATOR_ENABLED`, `INGESTION_TELEGRAM_URL`, `KOL_BOT_TOKEN`, `ENCRYPTION_KEY`), `Dockerfile`, `docker-compose.yml` (DB `onchain_bot_kol_system` + redis), `GET /api/health`. Must NOT lógica de negocio.
...
- [x] 3. Shared kernel + config + guards (Ph2 spec)
     ...
     Acceptance criteria: `npx jest apps/kol-calls/src/shared --coverage` sin fallos; `grep -r "from 'typeorm'\|from 'axios'" apps/kol-calls/src/shared/kernel apps/kol-calls/src/shared/domain` vacío
...
- [x] 4. Ingestion kol-type por HTTP+SSE (Ph3 spec + P3)
     ...
     Acceptance criteria: `curl -s 'localhost:3031/api/feed/sources?type=kol' | jq length` >= 0; test doble-delivery verde; `grep -ri "crypto-news" apps/kol-calls/src/ingestion` vacío (assert negativo P10)
...
- [x] 18. Deprecar ingestión KOL backend (P18, companion del 4)
      What to do / Must NOT do: Tras verificar el todo 4, marcar `@deprecated` (headers + JSDoc con puntero `apps/kol-calls/src/ingestion/`) en la contraparte backend (`apps/backend/src/telegram/ingestion/kol/` o equivalente real que el worker resuelva con lsp_find_references; si no existe contraparte directa, deprecation header en el consumer más cercano). El código backend sigue funcionando (dual-run). Tests legacy verdes. Must NOT borrar nada ni cambiar comportamiento.
      ...
      Commit: Y | chore(kol-system): depreca ingestión KOL backend (apunta a apps/kol-calls)
...
- [x] 6. Parsing a ficha estructurada preservando menciones (Ph5 spec + P5)
     ...
     Acceptance criteria: `npx jest apps/kol-calls/src/parsing` verde + invariante 1:1 en test
...
- [x] 8. Enrichment vía MarketDataPort dual (Ph7 spec + C-DATA-01 + G-17)
     ...
     Acceptance criteria: `USE_DATA_SERVICE_API=false npx jest apps/kol-calls/src/enrichment` verde; con `=true` usa HTTP (test con mock server)
...
- [x] 9. Scoring + classification-config por template (Ph8 spec + P6 + G-08)
     ...
     Acceptance criteria: `grep -r "classified_calls" apps/kol-calls/src` vacío; suite scoring verde
...
- [x] 14. Frontend: tabla caller|call|mc at|tracking|time ago|more details+ (P5/P8 + C-UX-01)
...
- [ ] 16. Cutover por flags + deprecate-only Tramo 1 (C4-bis: borrado solo en FINAL REVIEW)
      What to do / Must NOT do: Staging 48h validación (`KOL_PIPELINE_ENABLED=false`, `KOL_SYSTEM_ENABLED=true`) → prod Blue/Green (deploy inactivo, health, `TEMPLATE_ORCHESTRATOR_ENABLED=true`, observar 30min+2h, disable backend) → deprecate-only: headers `@deprecated` + JSDoc (nueva ruta `apps/kol-calls/...` + refactor target) en TODO el legacy KOL backend (`kol/`, `ingestion/kol/`, `vip-calls/`); NADA de `rm -rf`, NADA de DROP (archivado/borrado solo en FINAL REVIEW central todo 9). Cutover flags se mantienen. Rollback <30min listo.
...
- [ ] 17. SUPERSEDED por P13 — NO ejecutar. Contenido movido a Tramo 3 todo 9 (`apps/dexter-onchain-bot/`).
      ...
      Acceptance criteria: `ls apps/kol-calls/src | grep -i lookup` vacío
...
- [x] 19. Retirar KOL_BOT_TOKEN de setup+validación (P23)
      ...
      References: .omo/drafts/mega-refactor-tramos.md (P23); apps/kol-calls/.env.example; apps/kol-calls/src/shared/config/app.config.ts (validateKolSystemConfig)
      Acceptance criteria: `grep -rn "KOL_BOT_TOKEN" apps/kol-calls --include="*.ts" | grep -v spec | wc -l` = 0 + `npx jest apps/kol-calls/src/shared` verde
...
- [x] 20. Templates env multi-entorno + scp a Oracle (P24)
      ...
      Acceptance criteria: `git check-ignore apps/kol-calls/.env.staging && git check-ignore apps/kol-calls/.env.production && echo IGNORED-OK`; `git ls-files apps/kol-calls/.env*` lista solo development + 2 templates
      QA scenarios: happy boot con cada template (valores dummy); failure secreto commiteado → pre-commit lo cazaría (verificar con `git log --all -S 'sk-' --oneline -- apps/kol-calls | wc -l` = 0). Evidence .omo/evidence/task-20-mega-refactor-kol-system.log
      Commit: Y | chore(kol-system): templates env multi-entorno (P24)
- [x] 21. AGENTS.md vivo de kol-system (P25)
      What to do / Must NOT do: Crear `apps/kol-calls/AGENTS.md` espejo de `apps/backend/AGENTS.md` en estructura (overview, comandos, envs, puertos, convenciones TS/ESLint, tests, decisiones P1–P24 aplicables con fecha) adaptado a kol-system; registrar REGLA standing: cada todo futuro cierra con paso "actualiza AGENTS.md si cambió algo (comandos, envs, puertos, decisiones)". Verificar enlaces a ficheros reales (0 links rotos). Must NOT copiar secciones backend que no apliquen (MTProto, providers).
      ...
      Acceptance criteria: `test -f apps/kol-calls/AGENTS.md && grep -c "P1[0-9]" apps/kol-calls/AGENTS.md` >= 5 (decisiones pivot citadas)
...
- [x] 22. Scoring configurable por template (P28, follow-up del 9)
      ...
      References: .omo/drafts/mega-refactor-tramos.md (P28); apps/kol-calls/src/scoring/ (fórmula v1 como defaults); apps/kol-calls/src/templates/ (modelo a extender)
...
- [x] 23. Auth anti-exploit kol-system (P50)
      ...
      References: .omo/drafts/mega-refactor-tramos.md (P50); apps/kol-calls/src/templates/ + src/telegram/
...
- `apps/kol-calls/` en prod publicando VIP calls con `KOL_PIPELINE_ENABLED=false`.
