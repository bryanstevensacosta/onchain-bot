# add-feed-source - Work Plan (✅ COMPLETE)
...
**What it will NOT do:** Will not auto-join channels that the bot isn't a member of (placeholder title is used, mirroring the seeder fallback). Will not change existing GET endpoints or the existing feed page layout. Will not add channels with non-numeric IDs (validation matches the domain rule).

**Effort:** Short (4-6 hours)
**Risk:** Low - small surface area, well-defined pattern to follow, existing use case to call
**Decisions to sanity-check:** (1) Modal asks for ONLY channelId (matches add-kol UX, auto-resolves title/handle from Telegram). (2) Backend activates the source immediately after register (matches seeder, so channel starts ingesting right away). (3) Use `feedKeys.all` invalidation (covers both sources list + messages list refresh).

Your next move: approve and `$start-work` to execute, or run a high-accuracy review first.

---

> TL;DR (machine): Short effort, Low risk. Add POST /feed/sources + frontend Add Source modal mirroring Add KOL pattern.

## Scope

### Must have

- POST /feed/sources endpoint that registers a FeedSource via RegisterNewsSourceUseCase and activates it
- FeedMetadataResolver service extracted from seeder for shared use
- Frontend `features/add-feed-source/` (api + model + ui + index + 2 tests)
- ENDPOINTS.cryptoNews.sources.add entry
- Add Source button + modal wired into pages/feed/index.tsx
...
- No changes to entities/feed barrel exports (no new types exported)
- No new shared UI primitives (Button + Modal reused as-is)
- No new route (button lives on existing /feed page)
...
- Evidence directory: .omo/evidence/task-<N>-add-feed-source.<ext>
...
- [x] 1. Create `FeedMetadataResolver` service (extract from seeder)
     What to do:
  - Create new file `apps/backend/src/telegram/ingestion/feed/application/services/feed-metadata-resolver.service.ts`
  - Class `FeedMetadataResolver` — single public method `resolve(channelId: string): Promise<{ title: string; handle: string | null; needsManualJoin: boolean }>`
  - Move the body of `FeedSeeder.resolveMetadata()` (lines 125-160) into this class
  - Inject `TelegramListenerPort` and `Logger` via constructor
  - Update `FeedSeeder` to inject `FeedMetadataResolver` and replace its private `resolveMetadata()` with a delegation
  - Add `FeedMetadataResolver` to `FeedIngestionModule.providers` and exports
    Must NOT do:
  ...
  - apps/backend/src/telegram/ingestion/feed/infrastructure/seeders/feed.seeder.ts:125-160 (source of resolveMetadata)
  ...
  - apps/backend/src/telegram/ingestion/feed/feed-ingestion.module.ts:121 (existing provider registration pattern)
    Acceptance criteria:
  - File compiles cleanly (`npx tsc --noEmit` exits 0)
  - `FeedSeeder.seed()` still produces the same output for the same seed list
  ...
  - Constructor injection works (NestJS resolves `TelegramListenerPort` via DI)
    QA scenarios:
  - Happy: existing feed seeder spec still passes (`npm run test:backend -- --testPathPattern=feed.seeder.spec`)
  ...
  - Evidence: .omo/evidence/task-1-seeder-spec.log
    Commit: Y | refactor(feed): extract metadata resolver from seeder
...
- [x] 2. Add `POST /crypto-news/sources` endpoint to `FeedController`
     What to do:
  - Modify `apps/backend/src/telegram/ingestion/feed/api/http/feed.controller.ts`: - Import `Post`, `Body`, `HttpCode`, `HttpStatus` from `@nestjs/common` - Import `RegisterNewsSourceUseCase` from `../application/handlers/register-news-source.use-case` - Import `FeedSourceRepository` from `../application/ports/feed-source.repository` (already injected) - Import `FeedMetadataResolver` from `../application/services/feed-metadata-resolver.service` - Add `RegisterNewsSourceInput` import - Inject both new deps in constructor - Add `@Post('sources')` handler: - `@HttpCode(HttpStatus.CREATED)` (201) - Body: `{ channelId: string; handle?: string; title?: string }` (title optional — if absent, resolver fills it) - Resolve metadata: if `input.title` not provided OR empty, call `resolver.resolve(input.channelId)` to get `{ title, handle }`. If `input.title` is provided, prefer it (user override). - Call `registerSource.execute({ channelId, handle: input.handle ?? resolved.handle, title: input.title ?? resolved.title })` - Call `source.activate(); await sourceRepo.save(source);` to start ingestion (mirror seeder) - Map domain entity to `FeedSourceView` shape (same as `listSources`): `{ channelId, handle, title, isActive, lifecycleStatus, addedAt }` - Return the view
    Must NOT do:
  ...
  - Do NOT swallow the CONFLICT error — let `DomainErrorFilter` translate it to 409
    ...
    References:
  - apps/backend/src/telegram/ingestion/feed/api/http/feed.controller.ts:1-230 (full file)
  - apps/backend/src/telegram/ingestion/feed/application/handlers/register-news-source.use-case.ts:7-47
  - apps/backend/src/telegram/ingestion/feed/domain/entities/feed-source.entity.ts:111-114 (activate method)
  ...
  - `@Post('sources')` handler exists with signature `(body: { channelId: string; handle?: string; title?: string }): Promise<FeedSourceView>`
  - Response body matches `FeedSourceView` shape exactly
  ...
  - Failure: POST with non-numeric channelId returns 400 (caught by FeedSource.create validation)
  - Evidence: .omo/evidence/task-2-controller-spec.log
    Commit: Y | feat(feed): add POST /feed/sources endpoint

### Wave 2: Backend tests + frontend feature (parallel after Wave 1)

- [x] 3. Add `feed.controller.spec.ts` for POST endpoint
     What to do:
  - Create new test file `apps/backend/src/telegram/ingestion/feed/api/http/__tests__/feed.controller.spec.ts`
  - Use NestJS Testing module; mock `RegisterNewsSourceUseCase`, `FeedSourceRepository`, `FeedSourceMessageRepository`, `TelegramMtprotoListenerAdapter`, `Repository<FeedMessageMediaEntity>`, `FeedMetadataResolver`
  ...
  - apps/backend/src/telegram/ingestion/feed/infrastructure/seeders/**tests**/feed.seeder.spec.ts (reference mock style)
  - apps/backend/src/telegram/ingestion/feed/domain/entities/feed-source.entity.ts:42-76 (create signature)
    Acceptance criteria:
  - `npm run test:backend -- --testPathPattern=feed.controller.spec` passes all 6 cases
  - Test file co-located at `__tests__/feed.controller.spec.ts` per project convention
...
- [x] 4. Create `features/add-feed-source/` feature folder (api client + mutation + modal + barrel)
     What to do:
  - Create `apps/frontend/src/features/add-feed-source/api/add-feed-source-client.ts`:
    - Exports `addFeedSource({ channelId }: { channelId: string }): Promise<FeedSourceView>`
    - Calls `httpPost<{ channelId: string }, FeedSourceView>(ENDPOINTS.cryptoNews.sources.add, { channelId })`
  - Create `apps/frontend/src/features/add-feed-source/model/use-add-feed-source.ts`:
    - Exports `useAddFeedSource()` hook
    - Uses `useMutation<FeedSourceView, Error, { channelId: string }>`
    - On success: invalidates `feedKeys.all` (covers both `sources()` and `messages()`)
    - Imports `feedKeys` from `@/entities/feed`
  - Create `apps/frontend/src/features/add-feed-source/ui/add-feed-source-modal.tsx`:
  ...
  - Create `apps/frontend/src/features/add-feed-source/index.ts`: - Exports `AddFeedSourceModal` from `./ui/add-feed-source-modal` - Exports `useAddFeedSource` from `./model/use-add-feed-source`
    Must NOT do:
  ...
  - Do NOT add new shared types — reuse `FeedSource` from `@/entities/feed/api/feed-queries`
  ...
  - apps/frontend/src/entities/feed/api/feed-queries.ts (FeedSource type + feedKeys)
  ...
  - `AddFeedSourceModal` renders, accepts user input, submits via mutation
  - `useAddFeedSource` invalidates `feedKeys.all` on success
  ...
  - Evidence: .omo/evidence/task-4-modal-build.log
    Commit: Y | feat(frontend): add AddFeedSourceModal feature
...
- [x] 6. Wire button + modal into `pages/feed/index.tsx`
     What to do:
  - Modify `apps/frontend/src/pages/feed/index.tsx`: - Import `AddFeedSourceModal` and `Button` from `@/shared/ui` - Add `const [showAddModal, setShowAddModal] = useState(false);` near existing state - Modify header to be flex with title left + button right (mirror `pages/kols/index.tsx:112-127`):
    `tsx
      ...
      ` - Render `<AddFeedSourceModal isOpen={showAddModal} onClose={() => setShowAddModal(false)} />` after the header
    Must NOT do:
  ...
  - Do NOT change the page's existing data-fetching logic
    ...
    References:
  - apps/frontend/src/pages/feed/index.tsx:18-25 (current header)
  ...
  - Evidence: .omo/evidence/task-6-page-screenshot.png (or component test)
    Commit: Y | feat(feed-page): wire Add Source button + modal

- [x] 7. Add frontend tests for client + modal
     What to do:
  - Create `apps/frontend/src/features/add-feed-source/api/__tests__/add-feed-source-client.test.ts`:
    - Mock `@/shared/api/http-client` `httpPost`
    - Test: `addFeedSource({ channelId: '123' })` calls `httpPost('/crypto-news/sources', { channelId: '123' })` and returns the response
  - Create `apps/frontend/src/features/add-feed-source/ui/__tests__/add-feed-source-modal.test.tsx`: - Mock `useAddFeedSource` - Test cases: 1. Renders modal title "Add Source" 2. Submits with channelId, calls mutate, closes on success 3. Disables submit when channelId empty 4. Disables submit when channelId non-numeric 5. Shows error message when mutation fails 6. Doesn't close modal when mutation is pending 7. Resets input on close
    Must NOT do:
  ...
  - apps/frontend/src/test/setup.ts (vitest setup, if exists)
    Acceptance criteria:
  - `npm run test:frontend -- --run --reporter=verbose add-feed-source` passes all tests
  - Each test is independent and mocks `httpPost` / `useAddFeedSource`
...
- [x] 8. Verify everything end-to-end + run pre-commit hooks
  ...
  - Manual smoke test: navigate to `http://localhost:5173/feed` → click "+ Add Source" → enter valid channelId → submit → verify source appears in filter
  ...
  - Do NOT merge without all checks passing
    ...
    References:
  - apps/frontend/src/pages/feed/**tests**/feed-page.test.tsx (existing page test must still pass)
...
1. `refactor(feed): extract metadata resolver from seeder` (todo 1)
2. `feat(feed): add POST /feed/sources endpoint` (todo 2 + controller test in todo 3 — combined into one commit since the test validates the endpoint)
3. `feat(frontend): add AddFeedSourceModal feature` (todo 4 + todo 7 tests + todo 5 ENDPOINTS — combined since the feature is incomplete without its tests and endpoint entry)
4. `feat(feed-page): wire Add Source button + modal` (todo 6)
