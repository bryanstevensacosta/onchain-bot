...
- [ ] 1. Fix `require()` error in `ingestion-coordinator.service.spec.ts:19`
  ...
  1. **Acceptable fallback**: add `// eslint-disable-next-line @typescript-eslint/no-require-imports` with a justification comment explaining the Jest CJS subpath workaround (same comment as the existing header).
  1. **NOT acceptable**: static `import { IngestionCoordinator } from 'telegram/ingestion/shared/application/ingestion-coordinator.service'` — this re-triggers the Jest CJS mapping error at module-load time.
...
Parallelization: Wave 1 | Blocked by: None | Blocks: None
References:`apps/backend/src/shared/common/cache/redis.service.ts:40-49`Acceptance criteria:`cd apps/backend && npx eslint "src/shared/common/cache/redis.service.ts" 2>&1 | grep -c "warning"`must return 0
...
- [ ] 9. Fix `settings/settings.e2e-spec.ts` (18 warns) + 3 feed test files (3 warns)
  ...
  - `telegram/ingestion/feed/domain/entities/__tests__/feed-source.entity.spec.ts` (1 warn, line 1): `ErrorCode` imported but unused. Fix: remove the import.
  - `telegram/ingestion/feed/infrastructure/persistence/typeorm/mappers/__tests__/feed-message.mapper.spec.ts` (1 warn, line 2): `FeedMessageEntity` imported but unused. Fix: remove the import.
  - `telegram/ingestion/feed/infrastructure/persistence/typeorm/mappers/__tests__/feed-source.mapper.spec.ts` (1 warn, line 2): `FeedSourceEntity` imported but unused. Fix: remove the import.
  ...
  - `apps/backend/src/telegram/ingestion/feed/domain/entities/__tests__/feed-source.entity.spec.ts:1`
  - `apps/backend/src/telegram/ingestion/feed/infrastructure/persistence/typeorm/mappers/__tests__/feed-message.mapper.spec.ts:2`
  - `apps/backend/src/telegram/ingestion/feed/infrastructure/persistence/typeorm/mappers/__tests__/feed-source.mapper.spec.ts:2`
    ...
    Commit: Y | `fix(test): type app and remove unused imports in settings + feed specs`
