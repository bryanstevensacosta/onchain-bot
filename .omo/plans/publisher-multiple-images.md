...
- `BotApiFeedPublisherAdapter`: implement `sendMediaGroup` using Telegram Bot API `sendMediaGroup` with multipart + `attach://` references. Caption goes on `media[0]` only (Telegram limitation).
...
- NO changes to VIP calls publisher or its `TelegramPublisherPort` abstract class (keep `sendMediaGroup` as feed-only extension)
- NO changes to ingestion/media downloader (FeedMediaDownloader, extractMediaForMessage, etc.)
...
| W2   | 2     | Adapter: implement BotApiFeedPublisherAdapter.sendMediaGroup + buildMediaGroupMultipartBody           |
...
- [ ] 1. Add `imagePaths: string[]` to domain entity + `sendMediaGroup` to publisher port
  ...
  - `apps/backend/src/telegram/shared/domain/ports/telegram-publisher.port.ts:25-43` (port abstract class)
    ...
    **Commit:** Y | `feat(feed-publisher): add imagePaths to domain + sendMediaGroup to port`

### Wave 2 — Adapter

- [ ] 2. Implement `sendMediaGroup` in BotApiFeedPublisherAdapter
     **What to do:**
  ...
  - Export `buildMediaGroupMultipartBody()` pure function for testing
    ...
    **References:**
  - `apps/backend/src/telegram/crypto-news-publisher/infrastructure/senders/bot-api-feed-publisher.adapter.ts:119-175` (sendPhoto implementation as pattern)
  - `apps/backend/src/telegram/crypto-news-publisher/infrastructure/senders/bot-api-feed-publisher.adapter.ts:239-307` (postMultipart helper)
  - `apps/backend/src/telegram/crypto-news-publisher/infrastructure/senders/bot-api-feed-publisher.adapter.ts:315-349` (buildMultipartBody helper)
    **Acceptance:** `npx tsc --noEmit` on backend
    **QA:** Unit test for `sendMediaGroup` with 2 images
    **Parallelization:** Wave 2 | Blocked by: 1 | Blocks: 4
    **Commit:** Y | `feat(feed-publisher): implement sendMediaGroup in BotApiFeedPublisherAdapter`
...
- [ ] 3. Update `EnqueueMatchingMessageUseCase` to collect all image paths
  ...
  - Change `firstImagePath()` method to collect ALL paths:
    ```ts
    private collectImagePaths(message: FeedMessage): string[] {
  ...
  - Update `execute()` to pass `imagePaths` to `PublisherQueueEntry.create()`
    **Must NOT do:** Don't import anything new — `FeedMedia` already has `filePath`.
    **References:**
  ...
  - `apps/backend/src/telegram/crypto-news-publisher/application/handlers/enqueue-matching-message.use-case.ts:69-80` (create call)
    ...
    **Commit:** Y | `feat(feed-publisher): collect all image paths in enqueue use case`
...
- [ ] 4. Update `ProcessNextQueuedArticleUseCase.dispatchToTelegram` to use sendMediaGroup
  ...
  - `apps/backend/src/telegram/crypto-news-publisher/application/handlers/process-next-queued-article.use-case.ts:132-145` (dispatchToTelegram)
    ...
    **Commit:** Y | `feat(feed-publisher): use sendMediaGroup when entry has multiple images`
...
- [ ] 5. Update TypeORM entity, mapper, controller for imagePaths
  ...
  - `apps/backend/src/telegram/crypto-news-publisher/api/http/queue.controller.ts:150-171` (toView)
    ...
    **Commit:** Y | `feat(feed-publisher): add imagePaths to persistence and API`
...
- [ ] 6. Update frontend QueueEntryView type
  ...
  - Update test data in `feed-page.test.tsx` to include `imagePaths: []`
    **References:**
  - `apps/frontend/src/features/feed-publisher/api/queue-api.ts:14`
  - `apps/frontend/src/pages/feed/__tests__/feed-page.test.tsx:554-583`
    **Acceptance:** `npx tsc --noEmit` on frontend
    **Parallelization:** Wave 5 | Blocked by: 5 | Can parallelize with: 7
    **Commit:** Y | `feat(feed-publisher): add imagePaths to frontend API types`
...
- [ ] 7. Update frontend queue-view to display all images
  ...
  - Backward compat: use `entry.imagePaths` instead of `entry.imagePath`. The backend's `toView()` ensures `imagePaths` is always present (even if empty).
    ...
    **References:**
  - `apps/frontend/src/features/feed-publisher/ui/queue-view.tsx:82-89` (image display)
    **Acceptance:** `npx tsc --noEmit` on frontend
    **QA:** Visual check via Playwright
    **Parallelization:** Wave 5 | Blocked by: 6
    **Commit:** Y | `feat(feed-publisher): display all images in queue view`
...
- Conventional commit format: `feat(feed-publisher): <description>`
- Each commit with test changes (tests co-located or in **tests**/)
