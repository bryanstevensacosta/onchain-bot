...
- [x] 5. Swap cuenta dev-local (nueva, 3-4 canales) — REQUIERE OPERADOR (gen-session)
     Estado 2026-09-22: triple ACTUAL de dev movida a `.env.staging`; `.env` dev con triple NUEVA (operador); 1 canal `@cointelegraph` (-1001072723547, feed) verificado con mensajes reales fluyendo; muertos eliminados. COMPLETO.
...
- [x] 10. Dedup semántico cableado en enqueue (BLOCKED real)
      ...
      Acceptance criteria: specs — duplicado semántico → BLOCKED con refs (assert campos), único → PENDING + fingerprint guardado (assert store), modelo caído → enqueue sigue (fail-open spec); `npx jest deduplication feed-publisher` verde; `npx tsc --noEmit` backend limpio; prueba viva en staging (forzar duplicado real oferta: publicar 2 veces el mismo contenido vía API y ver 2º BLOCKED — diseñar el probe en evidence).
