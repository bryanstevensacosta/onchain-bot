...
**What you'll get:** Un flag `USE_MOCK_AI=true` que al activarlo en dev/staging hace que el pipeline de feed funcione completo (keywords, queue, throttle, daily cap, Telegram publish) pero **sin consumir créditos de IA** — en lugar de generar un post con LLM, publica el contenido original tal cual llegó del canal de Telegram.

**Why this approach:** Es la mínima intervención posible — un early return en `FeedLlmAdapter.generateForEntry()` cuando la env var está seteada. Todo lo demás (queue, throttle, daily cap, Telegram dispatch) corre idéntico a producción. El flag se lee directo de `process.env` porque es dev tooling, no merece registro formal en AppConfig.
...
> TL;DR (machine): Quick | Low risk | Early return in FeedLlmAdapter.generateForEntry() when USE_MOCK_AI=true, returns entry.rawContent without calling LLM. Tests + env files.

## Scope

### Must have

- Implementar flag `USE_MOCK_AI` en `FeedLlmAdapter.generateForEntry()` que retorna `entry.rawContent` sin llamar al LLM
...
- Wave 1 (todo 1): Implementación en `FeedLlmAdapter` + test (se pueden hacer juntos por ser el mismo archivo y spec)
...
- [ ] 1. Implementar mock AI en FeedLlmAdapter + tests
     What to do / Must NOT do:
  - En `feed-llm.adapter.ts`, al inicio de `generateForEntry()`, verificar `process.env.USE_MOCK_AI === 'true'`
  ...
  - En `feed-llm.adapter.spec.ts`, añadir 3 tests:
    ...
    References:
  - `apps/backend/src/telegram/crypto-news-publisher/infrastructure/llm/feed-llm.adapter.ts:51-136` (generateForEntry completo)
  - `apps/backend/src/telegram/crypto-news-publisher/infrastructure/llm/feed-llm.adapter.spec.ts:137-349` (tests existentes de generateForEntry)
  - Return type en `feed-llm.adapter.ts:51-58`
    Acceptance criteria (agent-executable):
  - `USE_MOCK_AI=true yarn --cwd apps/backend jest --testPathPattern='feed-llm.adapter.spec' --no-coverage` pasa los 3 tests nuevos + todos los existentes
  - `USE_MOCK_AI=false yarn --cwd apps/backend jest --testPathPattern='feed-llm.adapter.spec' --no-coverage` sigue pasando (modo normal no afectado)
  ...
  - Evidence: `.omo/evidence/task-1-mock-ai-dev-staging.txt`
    Commit: Y | `feat(feed-publisher): add USE_MOCK_AI flag for dev/staging without AI credits`
...
- [ ] 2. Añadir USE_MOCK_AI a env files
  ...
  - En `.env.staging.template`, **añadir la sección completa de feed publisher** que actualmente falta:
    ```
  ...
  - NO modificar ningún otro valor en los env files existentes
    ...
    References:
  - `apps/backend/.env.dev:103-105` (feed publisher section)
...
- [ ] F2. Code quality review — revisar que el early return en FeedLlmAdapter no introduce side effects, que los tests cubren mock mode y normal mode
- [ ] F3. Real manual QA — ejecutar `cd apps/backend && USE_MOCK_AI=true npx jest --testPathPattern='feed-llm.adapter.spec' --no-coverage` y verificar que pasa
- [ ] F4. Scope fidelity — git diff para confirmar que solo se tocaron: feed-llm.adapter.ts, feed-llm.adapter.spec.ts, .env.dev, .env.staging.template

## Commit strategy

- Un solo commit con los cambios de código + tests + env files
- Tipo: `feat(feed-publisher):`
- Mensaje: `feat(feed-publisher): add USE_MOCK_AI flag to skip AI generation in dev/staging`
