...
- **P15 multi-dashboard por template (2026-09-24)**: cada template configura N dashboards (`template_dashboards`: id, template_id, type, title, filters JSONB, layout JSONB, position). Tipos v1: `calls-table` (P5), `kol-ranking` (P11), `gem-feed` (filtros P6), `tracking-board` (P8). API `GET /api/templates/:id/dashboards` + CRUD; frontend renderiza pestañas por dashboard con sus filtros; seed `vip-calls` trae calls-table + kol-ranking. Sin límite impuesto (worker pagina si >20).
