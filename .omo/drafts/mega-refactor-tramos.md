...
- **P19 avatar fuente de verdad permanente (2026-09-24)**: ingestion-telegram resuelve foto de perfil del channel (MTProto), la almacena PERMANENTE y la sirve a kol-system (rankings + dashboard). Cláusulas: EXCLUIDA del janitor 72h (retención solo messages+media de mensajes); `avatarUrl` en la proyección `GET /api/feed/sources` (además del endpoint dedicado); fetch-once con refresh manual; fallback placeholder.
