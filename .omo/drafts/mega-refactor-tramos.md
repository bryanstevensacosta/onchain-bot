...
- **P30 lecciones Tramo 1 → Tramo 2 (2026-09-25, solo diseño, sin código)**: (a) SSE con `x-api-key` desde el día 1 (twin la exige); (b) Dockerfile CMD `dist/main.js` (nest sourceRoot src, sin prefijo); (c) alias `npm run dev` + AGENTS.md vivo por app (P25); (d) `?type=crypto-news` en TODA lectura feed + drop client-side + guard SSE (P10); (e) env templates staging+prod desde setup + DBs `onchain_bot_*`; (f) lockfile sincronizado (npm ci rompe si no); (g) pre-push knowledge gate: cada cambio de app actualiza su AGENTS.md.
